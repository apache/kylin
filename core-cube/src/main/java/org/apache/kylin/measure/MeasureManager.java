...
public class MeasureManager {
    ...
    private MeasureManager(KylinConfig config) throws IOException {
        ...
        crud.reloadAll();
        Broadcaster.getInstance(config).registerListener(new MeasureSyncListener(), CUBE, MEASURE, CUBE_DESC, CUBE_MEASURE);
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length < 1 || !args[0].equals("init")) {
            System.out.println(errInputMsg);
            return;
        }
        switch (args[1]) {
            case "all":
    ...
    private static void initByProject(String projectName) throws IOException {
        KylinConfig config = KylinConfig.getInstanceFromEnv();
        System.out.println("Initialize all cube measure under project " + projectName);
    ...
    private static void initialCubeMeasure(String cubeName) throws IOException {
        ...
        if (null == cube) {
            System.out.println(String.format(Locale.ROOT, "Can't find cube by project: %s, cube: %s.", cubeName));
            return;
        }
        ...
        while (iter.hasNext()) {
            ...
            if (null != measureInStore) {
                System.out.println(m.getKey() + " already exist, won't initialize it. you can delete it first.");
