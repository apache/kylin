...
public class H2Database {
	private static final Logger logger = LoggerFactory
			.getLogger(H2Database.class);

	private static final String[] ALL_TABLES = new String[] { "test_cal_dt",
			"test_category_groupings", "test_kylin_fact",
			"test_seller_type_dim", "test_sites" };
	...
	private void loadH2Table(String tableName, String joinType)
			throws SQLException {
		MetadataManager metaMgr = MetadataManager.getInstance(config);
		TableDesc tableDesc = metaMgr.getTableDesc(tableName.toUpperCase());
		File tempFile = null;

		String fileNameSuffix = joinType.equalsIgnoreCase("default") ? "" : "."
				+ joinType;
		...
		try {
			...
			InputStream csvStream = metaMgr.getStore().getResource(
					normalPath + fileNameSuffix);
			if (csvStream == null) {
				csvStream = metaMgr.getStore().getResource(normalPath);
			} else {
				logger.info("H2 decides to load "
						+ (normalPath + fileNameSuffix) + " for table "
						+ tableDesc.getName());
			}
	...
	private String generateCreateH2TableSql(TableDesc tableDesc,
			String csvFilePath) {
	...
	private static String getH2DataType(String javaDataType) {
		String hiveDataType = javaToH2DataTypeMapping.get(javaDataType
				.toLowerCase());
