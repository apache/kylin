...
public class SRowProjector {
	...
	public static SRowProjector fromColumns(final CubeSegment cubeSegment,
			final Cuboid cuboid, final Collection<TblColRef> dimensionColumns) {
		...
		RowKeyEncoder rowKeyMaskEncoder = new RowKeyEncoder(cubeSegment, cuboid) {
			...
			protected void fillColumnValue(TblColRef column, int columnLen,
				...
				byte bits = dimensionColumns.contains(column) ? (byte) 0xff
						: 0x00;
		...
		byte[] mask = rowKeyMaskEncoder.encode(new byte[cuboid.getColumns()
				.size()][]);
	...
	public class AggrKey implements Comparable<AggrKey> {
		...
		public AggrKey copy() {
			...
			System.arraycopy(this.data, this.offset, copy.data, copy.offset,
					length());
			return copy;
		}
		...
		public int compareTo(AggrKey o) {
			...
			for (int i = 0, j = offset, k = o.offset; i < n; i++, j++, k++) {
				if (groupByMask[i] != 0) {
					comp = BytesUtil.compareByteUnsigned(this.data[j],
							o.data[k]);
					if (comp != 0)
						return comp;
				}
