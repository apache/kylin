...
class PartitionFlatTableStageTest extends SparderBaseFunSuite with PrivateMethodTester
  ...
  test("test materializedFactTableView") {
    ...
    val exec = new PartitionMaterializeFactView(segmentJob, seg, params)
    exec.persistFactView()
    assertNull(params.getFlatTableDesc)
    assertNotNull(params.getTableDesc)
    assertNull(params.getSpanningTree)
    assertNotNull(params.getPartitionSpanningTree)
    assertNotNull(params.getFactTable)
    assertNotNull(params.getPartFactTable)
  }
