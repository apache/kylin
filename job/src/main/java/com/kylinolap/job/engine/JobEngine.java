...
public class JobEngine implements ConnectionStateListener {
	...
	public static JobEngine getInstance(String engineID,
			JobEngineConfig engineCfg) throws JobException {
	...
	private JobEngine(String engineID, JobEngineConfig context)
			throws JobException {
		if (context.getZookeeperString() == null
				|| context.getZookeeperString().equals("")) {
			throw new IllegalArgumentException(
					"Zookeeper connection string is null or empty");
		}
		...
		this.zkClient = CuratorFrameworkFactory.newClient(
				context.getZookeeperString(), retryPolicy);
		this.zkClient.start();
	...
	private void releaseLock() {
		try {
			...
			if (zkClient.getState().equals(CuratorFrameworkState.STARTED)) {
				...
				if (zkClient.checkExists().forPath(
						ZOOKEEPER_LOCK_PATH + "/" + this.engineID) != null) {
	...
	public void start(int daemonJobIntervalInSeconds) throws Exception {
		...
		sharedLock = new InterProcessMutex(zkClient, ZOOKEEPER_LOCK_PATH + "/"
				+ this.engineID);
		...
		zkClient.setData().forPath(ZOOKEEPER_LOCK_PATH + "/" + this.engineID,
				Bytes.toBytes(this.engineID));
		startScheduler();
	}
	...
	private void startScheduler() throws JobException, IOException {
		...
		this.scheduler.scheduleFetcher(this.daemonJobIntervalInSeconds,
				this.engineConfig);
	}
	...
	public void interruptJob(JobInstance jobInstance, JobStep jobStep)
	...
	public String getPrimaryEngineID() throws Exception {
		byte[] data = zkClient.getData().forPath(
				ZOOKEEPER_LOCK_PATH + "/" + this.engineID);
		if (data == null) {
			return "";
		} else {
