...
public class RowKeyDecoderTest extends LocalFileMetadataTestCase {
	...
	public void testDecodeWithoutSlr() throws Exception {
		CubeInstance cube = CubeManager.getInstance(this.getTestConfig())
				.getCube("TEST_KYLIN_CUBE_WITHOUT_SLR_READY");
		...
		assertEquals(
				"[CAL_DT, LEAF_CATEG_ID, META_CATEG_NAME, CATEG_LVL2_NAME, CATEG_LVL3_NAME, LSTG_FORMAT_NAME, LSTG_SITE_ID, SLR_SEGMENT_CD]",
				names.toString());
		assertEquals(
				"[2012-12-15, 11848, Health & Beauty, Fragrances, Women, FP-GTC, 0, 15]",
				values.toString());
	...
	public void testDecodeWithSlr() throws Exception {
		...
		byte[] key = { 0, 0, 0, 0, 0, 0, 1, -1, 49, 48, 48, 48, 48, 48, 48, 48,
				9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 11, 54, -105, 55, 13, 71, 114,
				65, 66, 73, 78, 9, 9, 9, 9, 9, 9, 9, 9, 0, 10, 0 };
		...
		assertEquals(
				"[SELLER_ID, CAL_DT, LEAF_CATEG_ID, META_CATEG_NAME, CATEG_LVL2_NAME, CATEG_LVL3_NAME, LSTG_FORMAT_NAME, LSTG_SITE_ID, SLR_SEGMENT_CD]",
				names.toString());
		assertEquals(
				"[10000000, 2012-01-02, 20213, Collectibles, Postcards, US StateCities & Towns, ABIN, 0, -99]",
				values.toString());
	...
	public void testEncodeAndDecodeWithUtf8() throws IOException {
		...
		AbstractRowKeyEncoder rowKeyEncoder = AbstractRowKeyEncoder
				.createInstance(cube.getFirstSegment(), baseCuboid);
		...
		assertEquals(
				...
				names.toString());
		assertEquals(
				"[2012-12-15, 11848, Health & Beauty, Fragrances, Women, 刊登格式, 0, 15]",
				values.toString());
	}
}
