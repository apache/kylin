...
public class NewBaseCuboidMapper<KEYIN> extends Mapper<KEYIN, Text, Text, Text> {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(NewBaseCuboidMapper.class);
	...
	protected void setup(Context context) throws IOException {
		cubeName = context.getConfiguration().get(BatchConstants.CFG_CUBE_NAME)
				.toUpperCase();
		segmentName = context.getConfiguration().get(
				BatchConstants.CFG_CUBE_SEGMENT_NAME);
	...
	private void prepareJoins() throws IOException {
			...
			if (join != null) {
				...
				if (!lookupTables.containsKey(lookupTableName)) {
					HiveTable htable = new HiveTable(metadataManager,
							lookupTableName);
					LookupBytesTable btable = new LookupBytesTable(
							metadataManager.getTableDesc(lookupTableName),
				...
				this.tableJoins.add(new TableJoin(joinType, fkIndice,
						lookupTableName, this
								.findColumnRowKeyRelationships(dim)));

			} else {

				this.factTblColAsRowKey.addAll(this
						.findColumnRowKeyRelationships(dim));
			}
		}
		...
		Collections.sort(tableJoins, new Comparator<TableJoin>() {
			...
			public int compare(TableJoin o1, TableJoin o2) {
				return Integer.valueOf(o2.fkIndice.size()).compareTo(
						Integer.valueOf(o1.fkIndice.size()));
			}
		});
	}

	private LinkedList<Pair<Integer, Integer>> findColumnRowKeyRelationships(
			DimensionDesc dim) {
		...
		for (TblColRef colRef : dim.getColumnRefs()) {
			...
			int rowKeyIndex = cubeDesc.getRowkey().getRowKeyIndexByColumnName(
					colRef.getName());
		...
		for (int i = 0; i < measureSize; i++) {
			...
			if (colRefs == null) {
				...
				for (int j = 0; j < colRefs.size(); j++) {
					...
					int factTblIdx = factTableDesc
							.findColumnByName(c.getName()).getZeroBasedIndex();
	...
	private byte[] buildKey(SplittedBytes[] splitBuffers) {
		...
		for (TableJoin tableJoin : this.tableJoins) {
			...
			for (int i = 0; i < tableJoin.fkIndice.size(); ++i) {
				rawKey[i] = new ByteArray(
						trimSplitBuffer(splitBuffers[tableJoin.fkIndice.get(i)]));
			}
			...
			if (dimRow == null) {
				...
				for (Pair<Integer, Integer> relation : tableJoin.dimTblColAsRowKey) {
					keyBytesBuf[relation.getSecond()] = dimRow[relation
							.getFirst()].data;
					filledDimension++;
				}
			}
		...
		for (Pair<Integer, Integer> relation : this.factTblColAsRowKey) {
			keyBytesBuf[relation.getSecond()] = trimSplitBuffer(splitBuffers[relation
					.getFirst()]);
			filledDimension++;
		}
	...
	public void map(KEYIN key, Text value, Context context) throws IOException,
		...
		if (!byteRowDelimiterInferred)
			byteRowDelimiter = bytesSplitter.inferByteRowDelimiter(
		...
		try {
			...
			logger.error("", t);
			context.getCounter(BatchConstants.MAPREDUCE_COUTNER_GROUP_NAME,
