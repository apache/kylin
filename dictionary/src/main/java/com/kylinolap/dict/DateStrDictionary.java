...
public class DateStrDictionary extends Dictionary<String> {
	...
	static SimpleDateFormat getDateFormat(String datePattern) {
		...
		if (formatThreadLocal == null) {
			threadLocalMap.put(datePattern,
	...
	public static Date stringToDate(String str, String pattern) {
		...
		try {
			...
			throw new IllegalArgumentException("'" + str
					+ "' is not a valid date of pattern '" + pattern + "'", e);
		}
		return date;
	}
	...
	final protected int getIdFromValueImpl(String value, int roundFlag) {
		...
		if (id < 0 || id >= 16777216)
			throw new IllegalArgumentException("'" + value + "' encodes to '"
					+ id + "' which is out of range of 3 bytes");
	...
	final protected String getValueFromIdImpl(int id) {
		if (id < baseId)
			throw new IllegalArgumentException("ID '" + id
					+ "' must not be less than base ID " + baseId);
	...
	@Override
	final protected int getIdFromValueBytesImpl(byte[] value, int offset,
			int len, int roundingFlag) {
		try {
	...
	public boolean equals(Object o) {
		...
		return StringUtils.equals(this.pattern, that.pattern)
