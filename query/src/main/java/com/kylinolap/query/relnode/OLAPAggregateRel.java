 ...
 * @author xjiang
 */
public class OLAPAggregateRel extends AggregateRelBase implements OLAPRel,
		EnumerableRel {
	...
	public OLAPAggregateRel(RelOptCluster cluster, RelTraitSet traits,
	...
	@Override
	public AggregateRelBase copy(RelTraitSet traitSet, RelNode input,
			BitSet groupSet, List<AggregateCall> aggCalls) {
		try {
			return new OLAPAggregateRel(getCluster(), traitSet, input,
					groupSet, aggCalls);
			...
			for (AggregateCall aggCall : aggCalls) {
				...
				if (aggCall.isDistinct()) {
					throw new IllegalStateException(
							"Distinct count is only allowed in innermost sub-query.");
				}
			}
		}
	}
	...
	private TblColRef buildRewriteColumn(FunctionDesc aggFunc) {
		...
		if (aggFunc.needRewrite()) {
			...
			TableDesc table = this.context.firstTableScan.getOlapTable()
					.getSourceTable();
	...
	private void buildGroups() {
		...
		for (int i = getGroupSet().nextSetBit(0); i >= 0; i = getGroupSet()
				.nextSetBit(i + 1)) {
			Set<TblColRef> columns = inputColumnRowType
					.getSourceColumnsByIndex(i);
		...
		if (inputAggRow.getFieldCount() != outputAggRow.getFieldCount()) {
			for (RelDataTypeField inputField : inputAggRow.getFieldList()) {
				...
				if (outputAggRow.getField(inputFieldName, true) == null) {
					TblColRef column = this.columnRowType
							.getColumnByIndex(inputField.getIndex());
					this.context.metricsColumns.add(column);
				}
			}
		}
	...
	private AggregateCall rewriteAggregateCall(AggregateCall aggCall,
		...
		RelDataTypeField field = getChild().getRowType().getField(fieldName,
				true);
		...
		AggregateCall newAggCall = new AggregateCall(newAgg, false, newArgList,
				fieldType, newAgg.getName());
		...
		SqlAggFunction aggFunction = (SqlAggFunction) newAggCall
				.getAggregation();
		...
		return new AggregateCall(newAgg, false, newArgList, inferReturnType,
				newAgg.getName());
	}

	private Aggregation createHyperLogLogAggFunction(RelDataType returnType) {
		RelDataTypeFactory typeFactory = getCluster().getTypeFactory();
		SqlIdentifier sqlIdentifier = new SqlIdentifier("HLL_COUNT",
				new SqlParserPos(1, 1));
		AggregateFunction aggFunction = AggregateFunctionImpl
				.create(HLLDistinctCountAggFunc.class);
		List<RelDataType> argTypes = new ArrayList<RelDataType>();
		List<SqlTypeFamily> typeFamilies = new ArrayList<SqlTypeFamily>();
		for (FunctionParameter o : aggFunction.getParameters()) {
			final RelDataType type = o.getType(typeFactory);
			argTypes.add(type);
			typeFamilies.add(Util.first(type.getSqlTypeName().getFamily(),
					SqlTypeFamily.ANY));
		}
		return new SqlUserDefinedAggFunction(sqlIdentifier,
				ReturnTypes.explicit(returnType),
				InferTypes.explicit(argTypes),
				OperandTypes.family(typeFamilies), aggFunction);
	}
	...
	public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
		...
		try {
			enumAggRel = new EnumerableAggregateRel(getCluster(), getCluster()
					.traitSetOf(EnumerableConvention.INSTANCE), getChild(),
					this.groupSet, rewriteAggCalls);
		} catch (InvalidRelException e) {
			throw new IllegalStateException(
					"Can't create EnumerableAggregateRel!", e);
		}
