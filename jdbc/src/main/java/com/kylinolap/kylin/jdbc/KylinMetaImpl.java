...
public class KylinMetaImpl implements Meta {
	...
	private ResultSet mockEmptyResultSet() {
		...
		try {
			...
			resultSet = this.conn.getFactory().newResultSet(
					this.conn.createStatement(),
					new KylinPrepare.PrepareResult(null, null,
							new KylinEnumerator<Object[]>(data), ColumnMetaData
									.struct(columnMetas)),
	...
	public ResultSet getTables(String catalog, Pat schemaPattern,
		...
		if (null != metaProject) {
			...
			final NamedFieldGetter<MetaTable> tableGetter = new NamedFieldGetter<MetaTable>(
					MetaTable.class, tables.getMeta(), "TABLE_CAT",
			...
			try {
				resultSet = this.conn.getFactory().newResultSet(
						this.conn.createStatement(),
						new KylinPrepare.PrepareResult(null, null, null,
								tableGetter.structType) {
							@Override
							public Cursor createCursor() {
								return tableGetter.cursor(tables
										.getEnumerator());
							}
						}, this.conn.getTimeZone());
				KylinConnectionImpl.TROJAN.execute(resultSet);
	...
	public ResultSet getColumns(String catalog, Pat schemaPattern,
		...
		if (null != metaProject) {
			...
			final NamedFieldGetter<MetaColumn> columnGetter = new NamedFieldGetter<MetaColumn>(
					MetaColumn.class, columns.getMeta(), "TABLE_CAT",
					"TABLE_SCHEM", "TABLE_NAME", "COLUMN_NAME", "DATA_TYPE",
					"TYPE_NAME", "COLUMN_SIZE", "BUFFER_LENGTH",
					"DECIMAL_DIGITS", "NUM_PREC_RADIX", "NULLABLE", "REMARKS",
					"COLUMN_DEF", "SQL_DATA_TYPE", "SQL_DATETIME_SUB",
					"CHAR_OCTET_LENGTH", "ORDINAL_POSITION", "IS_NULLABLE",
					"SCOPE_CATALOG", "SCOPE_TABLE", "SOURCE_DATA_TYPE",
			...
			try {
				resultSet = this.conn.getFactory().newResultSet(
						...
						new KylinPrepare.PrepareResult(null, null, null,
							...
							@Override
							public Cursor createCursor() {
								return columnGetter.cursor(columns
										.getEnumerator());
							}
						}, this.conn.getTimeZone());
	...
	public ResultSet getSchemas(String catalog, Pat schemaPattern) {
		...
		if (null != metaProject) {
			...
			final NamedFieldGetter<MetaSchema> schemaGetter = new NamedFieldGetter<MetaSchema>(
					MetaSchema.class, schemas.getMeta(), "TABLE_SCHEM",
					"TABLE_CATALOG");
			...
			try {
				resultSet = this.conn.getFactory().newResultSet(
						this.conn.createStatement(),
						new KylinPrepare.PrepareResult(null, null, null,
								schemaGetter.structType) {
							@Override
							public Cursor createCursor() {
								return schemaGetter.cursor(schemas
										.getEnumerator());
							}
						}, this.conn.getTimeZone());
	...
	public ResultSet getCatalogs() {
		...
		if (null != metaProject) {
			...
			final NamedFieldGetter<MetaCatalog> catalogGetter = new NamedFieldGetter<MetaCatalog>(
					MetaCatalog.class, catalogs.getMeta(), "TABLE_CATALOG");
			...
			try {
				resultSet = this.conn.getFactory().newResultSet(
						this.conn.createStatement(),
						new KylinPrepare.PrepareResult(null, null, null,
								catalogGetter.structType) {
							@Override
							public Cursor createCursor() {
								return catalogGetter.cursor(catalogs
										.getEnumerator());
							}
						}, this.conn.getTimeZone());
	...
	public ResultSet getTableTypes() {
		...
		tableTypeMeta.add(ColumnMetaData.dummy(
	...
	public Cursor createCursor(AvaticaResultSet resultSet) {
		KylinPrepare.PrepareResult result = ((KylinResultSet) resultSet)
				.getPrepareResult();
	...
	public AvaticaPrepareResult prepare(AvaticaStatement statement, String sql) {
		...
		try {
			...
			throw new RuntimeException(
					"Failed to query kylin server with exception "
							+ e.getLocalizedMessage());
		}

		return new KylinPrepare.PrepareResult(sql, null,
				(Enumerator<Object[]>) result.getEnumerator(),
	...
	public static class MetaCatalog implements Named {
		...
		public int hashCode() {
			...
			int result = 1;
			result = prime * result
					+ ((tableCatalog == null) ? 0 : tableCatalog.hashCode());
			return result;
		}
	...
	public static class MetaTable implements Named {
		...
		public MetaTable(String tableCat, String tableSchem, String tableName,
				String tableType, String remarks, String typeCat,
				String typeSchem, String typeName,
	...
	public static class MetaColumn implements Named {
		...
		public MetaColumn(String tableCat, String tableSchem, String tableName,
				String columnName, int dataType, String typeName,
				int columnSize, int bufferLength, int decimalDigits,
				int numPrecRadix, int nullable, String remarks,
				String columnDef, int sqlDataType, int sqlDatetimeSub,
				int charOctetLength, int ordinalPosition, String isNullable,
				String scopeCatalog, String scopeTable, int sourceDataType,
	...
	private static class NamedFieldGetter<T> {
		...
		public NamedFieldGetter(Class<T> clazz, List<ColumnMetaData> columns,
				String... names) {
