            ...
            if (getter != null && setter != null) {
                ...
                if (params.length == 1 && params[0] == returnType) {
                    try {
                        ...
                        Assert.assertEquals(
                                String.format("Failed while testing property %s", prop.getName()), value,
                                getter.invoke(bean));

                    } catch (Exception ex) {
                        ex.printStackTrace();
                        System.err.println(String.format(
                                "An exception was thrown while testing the property %s: %s", prop.getName(),
                                ex.toString()));
                    }
                }
            }
        ...
        if (clazz.isArray()) {
            ...
            System.err
                    .println("Unable to build an instance of class " + clazz.getName()
                            + ", please add some code to the " + BeanValidator.class.getName()
                            + " class to do this.");
            return null;
        }
    }
