...
public class NDCuboidMapperTest extends LocalFileMetadataTestCase {
	...
	public void testMapReduceWithSlr() throws IOException {
		...
		mapReduceDriver.getConfiguration().set(
				BatchConstants.CFG_CUBE_SEGMENT_NAME, segmentName);
		...
		byte[] value = { 14, 7, 23, -16, 56, 92, 114, -80, 118, 14, 7, 23, -16,
				56, 92, 114, -80, 118, 14, 7, 23, -16, 56, 92, 114, -80, 118, 1 };
		Pair<Text, Text> input1 = new Pair<Text, Text>(new Text(key), new Text(
				value));
		...
		byte[] resultKey = { 0, 0, 0, 0, 0, 0, 1, 127, 49, 48, 48, 48, 48, 48,
				48, 48, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 55, 13, 71, 114, 65, 66,
				73, 78, 9, 9, 9, 9, 9, 9, 9, 9, 0, 10, 0 };
		byte[] resultValue = { 14, 7, 23, -16, 56, 92, 114, -80, 118, 14, 7,
				23, -16, 56, 92, 114, -80, 118, 14, 7, 23, -16, 56, 92, 114,
				-80, 118, 1 };
		...
		System.out
				.println(Bytes.toLong(new byte[] { 0, 0, 0, 0, 0, 0, 1, -1 }));
		for (int i = 0; i < result.size(); i++) {
			byte[] bytes = new byte[result.get(i).getFirst().getLength()];
			System.arraycopy(result.get(i).getFirst().getBytes(), 0, bytes, 0,
					result.get(i).getFirst().getLength());
