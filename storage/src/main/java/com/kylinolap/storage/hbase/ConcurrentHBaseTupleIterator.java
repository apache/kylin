...
public class ConcurrentHBaseTupleIterator implements ITupleIterator {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ConcurrentHBaseTupleIterator.class);
	...
	private static ListeningExecutorService executor = MoreExecutors
			.listeningDecorator(Executors.newFixedThreadPool(KylinConfig
					.getInstanceFromEnv().getConcurrentScanThreadCount()));
	...
	private BlockingQueue<Tuple> tupleQueue = new LinkedBlockingQueue<Tuple>(
			MAX_QUEUED_RESULTS);
	private List<ListenableFuture<Long>> scanFutures = new ArrayList<ListenableFuture<Long>>();
	private List<Throwable> scanExceptions = Collections
			.synchronizedList(new ArrayList<Throwable>());

	private Tuple next;

	public ConcurrentHBaseTupleIterator(HConnection conn,
			Map<CubeSegment, Collection<HBaseKeyRange>> segmentKeyRanges,
			CubeDesc cubeDesc, CubeInstance cube,
		...
		for (CubeSegment cubeSegment : this.segmentKeyRanges.keySet()) {
			for (HBaseKeyRange keyRange : this.segmentKeyRanges
					.get(cubeSegment)) {
				...
				for (RowValueDecoder rowValueDecoder : this.rowValueDecoders) {
					localRowValueDecoders.add(new RowValueDecoder(
							rowValueDecoder));
				}

				ListenableFuture<Long> scanFuture = executor
						.submit(new RangeScanCallable(cubeSegment, keyRange,
								localRowValueDecoders));
	...
	private boolean validate() {
		if (scanExceptions.size() > 0) {
			throw new RuntimeException(scanExceptions.get(0)
					.getLocalizedMessage());
		}
	...
	private class RangeScanCallable implements Callable<Long> {
		...
		public Long call() throws Exception {
			...
			try {
				...
				while (resultIterator.hasNext()) {
					...
					try {
						tupleQueue.put(translateResult(resultIterator.next(),
								tupleInfo));
					} catch (IOException e) {
						throw new IllegalStateException(
								"Can't translate result "
										+ resultIterator.next(), e);
					}
				}
				...
				throw new StorageException(
						"Error when open connection to table " + this.tableName,
						t);
			} finally {
				this.closeScanner();
				this.closeTable();
		...
		private final Iterator<Result> doScan(HBaseKeyRange keyRange) {
			...
			try {
				...
				scanner = CoprocessorEnabler.scanWithCoprocessorIfBeneficial(
						cubeSeg, keyRange.getCuboid(), filter, groupBy,
						rowValueDecoders, context, table, scan);
				iter = scanner.iterator();
			} catch (Throwable t) {
				String msg = MessageFormat
						.format("Error when scan from lower key {1} to upper key {2} on table {0}.",
								tableName,
								Bytes.toString(keyRange.getStartKey()),
		...
		private String logScan(HBaseKeyRange keyRange) {
			...
			sb.append("Scan ").append(tableName).append(" from ")
					.append(keyRange.getStartKeyAsString()).append(" to ")
					.append(keyRange.getStopKeyAsString()).append(" on ");
		...
		private Scan buildScan(HBaseKeyRange keyRange) {
			...
			for (RowValueDecoder valueDecoder : rowValueDecoders) {
				HBaseColumnDesc hbaseColumn = valueDecoder.getHBaseColumn();
				byte[] byteFamily = Bytes.toBytes(hbaseColumn
						.getColumnFamilyName());
				byte[] byteQualifier = Bytes
						.toBytes(hbaseColumn.getQualifier());
			...
			InclusiveStopFilter endFilter = new InclusiveStopFilter(
					keyRange.getStopKey());
		...
		private void applyFuzzyFilter(Scan scan, HBaseKeyRange keyRange) {
			...
			if (fuzzyKeys != null && fuzzyKeys.size() > 0) {
				...
				logger.info("Add FuzzyRowFilter: "
						+ keyRange.getFuzzyKeyAsString());
			}
		}
		...
		private TupleInfo buildTupleInfo(Cuboid cuboid) {
			...
			for (int i = 0; i < rowColumns.size(); i++) {
				...
				// add normal column
				info.setField(colNames.get(i), rowColumns.get(i), rowColumns
						.get(i).getDatatype(), index++);
			}

			// derived columns and filler
			Map<Array<TblColRef>, List<DeriveInfo>> hostToDerivedInfo = cubeSeg
					.getCubeDesc().getHostToDerivedInfo(rowColumns, null);
			for (Entry<Array<TblColRef>, List<DeriveInfo>> entry : hostToDerivedInfo
					.entrySet()) {
				...
				for (DeriveInfo deriveInfo : entry.getValue()) {
					...
					for (TblColRef derivedCol : deriveInfo.columns) {
						...
						info.setField(derivedField, derivedCol,
								derivedCol.getDatatype(), index++);
					}
					// add filler
					info.addDerivedColumnFiller(Tuple.newDerivedColumnFiller(
							rowColumns, hostCols, deriveInfo, info,
							CubeManager.getInstance(cube.getConfig()), cubeSeg));
				}
			}
		...
		private String getFieldName(TblColRef column,
				Map<TblColRef, String> aliasMap) {
			String name = null;
			if (aliasMap != null) {
		...
		private Tuple translateResult(final Result res, TupleInfo tupleInfo)
				throws IOException {
			...
			for (int i = 0; i < dimensionNames.size(); i++) {
				...
				tuple.setDimensionValue(dimensionNames.get(i),
						dimensionValues.get(i));
			}

			// derived
			for (IDerivedColumnFiller filler : tupleInfo
					.getDerivedColumnFillers()) {
		...
		private void closeScanner() {
			try {
				...
				throw new StorageException(
						"Error when close scanner for table " + tableName, t);
			}
		}
		...
		private void closeTable() {
			...
			try {
				...
				throw new StorageException("Error when close table "
						+ tableName, t);
			}
		}
	}
