...
public class RowKeyColumnIO {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(RowKeyColumnIO.class);
	...
	public void writeColumn(TblColRef column, byte[] value, int valueLen,
			byte dft, byte[] output, int outputOffset) {
		writeColumn(column, value, valueLen, 0, dft, output, outputOffset);
	}

	public void writeColumn(TblColRef column, byte[] value, int valueLen,
			int roundingFlag, byte dft, byte[] output, int outputOffset) {
		...
		try {
			...
			BytesUtil.writeUnsigned(id, output, outputOffset,
					dict.getSizeOfId());
		} catch (IllegalArgumentException ex) {
			for (int i = outputOffset; i < outputOffset + columnLen; i++)
				output[i] = dft;
			logger.error("Can't translate value "
					+ Bytes.toString(value, 0, valueLen)
					+ " to dictionary ID, roundingFlag " + roundingFlag
					+ ". Using default value " + String.format("\\x%02X", dft));
		}
	}
	...
	private byte[] padFixLen(int length, byte[] valueBytes) {
		...
		if (valLen == length) {
			...
			Arrays.fill(newValueBytes, valLen, length,
					RowConstants.ROWKEY_PLACE_HOLDER_BYTE);
	...
	public String readColumnString(TblColRef col, byte[] bytes, int bytesLen) {
		...
		if (dict == null) {
			...
			try {
				...
				logger.error("Can't get dictionary value for column "
						+ col.getName() + " (id = " + id + ")");
				return "";
			}
		}
	...
	public Dictionary<String> getDictionary(TblColRef col) {
		...
		return (Dictionary<String>) CubeManager.getInstance(
				seg.getCubeInstance().getConfig()).getDictionary(seg, col);
	}
