...
public class JobEngineConfig {
	private static final Logger logger = LoggerFactory
			.getLogger(JobEngineConfig.class);
	public static String HADOOP_JOB_CONF_FILENAME = "hadoop_job_conf_";

	public String getHadoopJobConfFilePath(CubeCapacity capaticy)
			throws IOException {
		String hadoopJobConfFile = (HADOOP_JOB_CONF_FILENAME
		...
		if (null == path || !new File(path).exists()) {
			...
			if (defaultFilePath.exists()) {
				...
				logger.debug("Search conf file " + hadoopJobConfFile
						+ "  from classpath ...");
				InputStream is = JobEngineConfig.class.getClassLoader()
						.getResourceAsStream(hadoopJobConfFile);
				if (is == null) {
					logger.debug("Can't get " + hadoopJobConfFile
							+ " from classpath");
	...
	public JobEngineConfig(KylinConfig kylinConfig) {
		...
		this.asyncJobCheckInterval = kylinConfig
				.getYarnStatusCheckIntervalSeconds();
