...
public class KylinTestBase {
	 ...
	 * @return
	 */
	protected static List<File> getFilesFromFolder(final File folder,
	...
	protected static void putTextTofile(File file, String sql)
			throws IOException {
	...
	protected static List<String> getParameterFromFile(File sqlFile)
			throws IOException {
	...
	protected static void printResult(ITable resultTable)
		...
		for (int i = 0; i < columnCount; i++) {
			sb.append(resultTable.getTableMetaData().getColumns()[i]
					.getColumnName());
			sb.append("-");
			sb.append(resultTable.getTableMetaData().getColumns()[i]
					.getDataType());
			sb.append("\t");
			columns[i] = resultTable.getTableMetaData().getColumns()[i]
					.getColumnName();
		}
		sb.append("\n");
	...
	protected ITable executeQuery(IDatabaseConnection dbConn, String queryName,
			String sql, boolean needSort) throws Exception {

		// change join type to match current setting
		sql = changeJoinType(sql, joinType);

		ITable queryTable = dbConn.createQueryTable(
				resultTableName + queryName, sql);
		String[] columnNames = new String[queryTable.getTableMetaData()
				.getColumns().length];
		for (int i = 0; i < columnNames.length; i++) {
			columnNames[i] = queryTable.getTableMetaData().getColumns()[i]
					.getColumnName();
		}
		if (needSort) {
	...
	protected int executeQuery(String sql, boolean needDisplay)
			throws SQLException {
	...
	protected ITable executeDynamicQuery(IDatabaseConnection dbConn,
			String queryName, String sql, List<String> parameters,
			boolean needSort) throws Exception {

		// change join type to match current setting
		sql = changeJoinType(sql, joinType);

		PreparedStatement prepStat = dbConn.getConnection().prepareStatement(
				sql);
	...
	protected static String changeJoinType(String sql, String targetType) {
		...
		for (int i = 0; i < tokens.length - 1; ++i) {
			if ((tokens[i].equalsIgnoreCase("inner") || tokens[i]
	...
	protected static void batchChangeJoinType(String targetType)
			throws IOException {
	...
	protected void verifyResultRowCount(String queryFolder) throws Exception {
		...
		for (File sqlFile : sqlFiles) {
			...
			int expectRowCount = Integer.parseInt(Files.readFirstLine(
					expectResultFile, Charset.defaultCharset()));

			// execute Kylin
			printInfo("Query Result from Kylin - " + queryName + "  ("
					+ queryFolder + ")");
			IDatabaseConnection kylinConn = new DatabaseConnection(
					cubeConnection);
	...
	protected void execAndCompQuery(String queryFolder,
			String[] exclusiveQuerys, boolean needSort) throws Exception {
	...
	protected void execAndCompDynamicQuery(String queryFolder,
			String[] exclusiveQuerys, boolean needSort) throws Exception {
		printInfo("---------- test folder: " + queryFolder);
		Set<String> exclusiveSet = buildExclusiveSet(exclusiveQuerys);
	...
	protected int runSQL(File sqlFile, boolean debug, boolean explain)
			throws Exception {
		if (debug) {
	...
	protected int output(ResultSet resultSet, boolean needDisplay)
			throws SQLException {
		int count = 0;
