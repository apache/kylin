...
public class HBaseUnionUtil {
    ...
    public static Connection getConnection(KylinConfig config, String identifier) throws IOException {
        final Connection masterConn = HBaseConnection.get(config.getStorageUrl());
        if (config.getSecondaryStorageUrl() == null) {
            return masterConn;
        }
        if (masterConn.getAdmin().isTableAvailable(TableName.valueOf(identifier))) {
            return masterConn;
        }
        logger.info("Table is not found in master cluster: " + config.getStorageUrl()
                + ", fallback to " + config.getSecondaryStorageUrl());
