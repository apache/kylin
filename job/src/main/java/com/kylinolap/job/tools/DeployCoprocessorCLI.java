...
public class DeployCoprocessorCLI {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(DeployCoprocessorCLI.class);
	...
	public static void main(String[] args) throws IOException {
		...
		Configuration hconf = HadoopUtil.newHBaseConfiguration(kylinConfig
				.getStorageUrl());
	...
	public static void setCoprocessorOnHTable(HTableDescriptor desc,
			Path hdfsCoprocessorJar) throws IOException {
		logger.info("Set coprocessor on " + desc.getNameAsString());
		desc.addCoprocessor(AGGR_COPROCESSOR_CLS_NAME, hdfsCoprocessorJar,
				1001, null);
	}

	public static void resetCoprocessor(String tableName,
			HBaseAdmin hbaseAdmin, Path hdfsCoprocessorJar) throws IOException {
	...
	private static List<String> resetCoprocessorOnHTables(
			HBaseAdmin hbaseAdmin, Path hdfsCoprocessorJar,
	...
	public static Path getNewestCoprocessorJar(KylinConfig config,
		...
		for (FileStatus fileStatus : fileSystem.listStatus(coprocessorDir)) {
			if (fileStatus.getPath().toString().endsWith(".jar")) {
				if (newestJar == null) {
					...
					if (newestJar.getModificationTime() < fileStatus
							.getModificationTime())
						newestJar = fileStatus;
				}
			}
		...
		Path path = newestJar.getPath()
				.makeQualified(fileSystem.getUri(), null);
	...
	public static Path uploadCoprocessorJar(String localCoprocessorJar,
		...
		for (FileStatus fileStatus : fileSystem.listStatus(coprocessorDir)) {
			if (fileStatus.getLen() == localCoprocessorJar.length()
					&& fileStatus.getModificationTime() == localCoprocessorFile
							.lastModified()) {
	...
	private static Set<String> getCoprocessorJarPaths(HBaseAdmin hbaseAdmin,
		...
		for (String tableName : tableNames) {
			...
			for (Map.Entry<ImmutableBytesWritable, ImmutableBytesWritable> e : tableDescriptor
					.getValues().entrySet()) {
				keyMatcher = HConstants.CP_HTD_ATTR_KEY_PATTERN.matcher(Bytes
						.toString(e.getKey().get()));
				if (!keyMatcher.matches()) {
					continue;
				}
				valueMatcher = HConstants.CP_HTD_ATTR_VALUE_PATTERN
						.matcher(Bytes.toString(e.getValue().get()));
