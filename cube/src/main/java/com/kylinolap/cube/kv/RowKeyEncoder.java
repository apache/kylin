...
public class RowKeyEncoder extends AbstractRowKeyEncoder {
	...
	public byte[] encode(byte[][] values) {
		...
		for (int i = 0; i < cuboid.getColumns().size(); i++) {
			...
			if (value == null) {
				...
				fillColumnValue(column, colLength, value, value.length, bytes,
						offset);
			}
			offset += colLength;
	...
	protected int fillHeader(byte[] bytes, byte[][] values) {
		int offset = 0;
		System.arraycopy(cuboid.getBytes(), 0, bytes, offset,
				RowConstants.ROWKEY_CUBOIDID_LEN);
		offset += RowConstants.ROWKEY_CUBOIDID_LEN;
		if (this.headerLength != offset) {
			throw new IllegalStateException("Expected header length is "
					+ headerLength + ". But the offset is " + offset);
		}
		return offset;
	}
	...
	protected void fillColumnValue(TblColRef column, int columnLen,
		...
		if (value == null) {
			...
			System.arraycopy(valueBytes, 0, outputValue, outputValueOffset,
					columnLen);
			return;
		}

		colIO.writeColumn(column, value, valueLen, this.blankByte, outputValue,
				outputValueOffset);
	}
