...
public class TrieDictionary<T> extends Dictionary<T> {
	...
	public static final byte[] HEAD_MAGIC = new byte[] { 0x54, 0x72, 0x69,
			0x65, 0x44, 0x69, 0x63, 0x74 }; // "TrieDict"
	...
	private void init(byte[] trieBytes) {
		...
		if (BytesUtil.compareBytes(HEAD_MAGIC, 0, trieBytes, 0,
				HEAD_MAGIC.length) != 0)
			throw new IllegalArgumentException(
					"Wrong file type (magic does not match)");

		try {
			DataInputStream headIn = new DataInputStream( //
					new ByteArrayInputStream(trieBytes, HEAD_SIZE_I,
			...
			if (converterName.isEmpty() == false)
				this.bytesConvert = (BytesConverter<T>) Class.forName(
						converterName).newInstance();

			this.nValues = BytesUtil.readUnsigned(trieBytes, headSize
	...
	final protected int getIdFromValueImpl(T value, int roundingFlag) {
		if (enableCache && roundingFlag == 0) {
			...
			if (cache != null) {
				...
				id = getIdFromValueBytes(valueBytes, 0, valueBytes.length,
						roundingFlag);
	 ...
	 */
	private int lookupSeqNoFromValue(int n, byte[] inp, int o, int inpEnd,
			int roundingFlag) {
		if (inp.length == 0) // special 'empty' value
			return checkFlag(headSize, BIT_IS_END_OF_VALUE) ? 0 : roundSeqNo(
					roundingFlag, -1, -1, 0);
		...
		while (true) {
			...
			if (o == inpEnd) {
				return p == end && isEndOfValue ? seq : roundSeqNo(
						roundingFlag, seq - 1, -1, seq); // input all matched
			}
			if (isEndOfValue)
				seq++;
			...
			int c = headSize
					+ (BytesUtil.readUnsigned(trieBytes, n, sizeChildOffset) & childOffsetMask);
	 ...
	 */
	private int lookupValueFromSeqNo(int n, int seq, byte[] returnValue,
			int offset) {
		int o = offset;
		while (true) {
			...
			int nValuesBeneath;
			while (true) {
				nValuesBeneath = BytesUtil.readUnsigned(trieBytes, c
						+ sizeChildOffset, sizeNoValuesBeneath);
	...
	public void readFields(DataInput in) throws IOException {
		byte[] headPartial = new byte[HEAD_MAGIC.length + Short.SIZE
				+ Integer.SIZE];
		in.readFully(headPartial);

		if (BytesUtil.compareBytes(HEAD_MAGIC, 0, headPartial, 0,
				HEAD_MAGIC.length) != 0)
			throw new IllegalArgumentException(
					"Wrong file type (magic does not match)");

		DataInputStream headIn = new DataInputStream( //
				new ByteArrayInputStream(headPartial, HEAD_SIZE_I,
						headPartial.length - HEAD_SIZE_I));
	...
	public static void main(String[] args) throws Exception {
		TrieDictionaryBuilder<String> b = new TrieDictionaryBuilder<String>(
