 ...
 * @author xjiang
 */
public class OLAPFilterRel extends FilterRelBase implements OLAPRel,
		EnumerableRel {
	...
	private static class TupleFilterVisitor extends RexVisitorImpl<TupleFilter> {
		...
		private CompareTupleFilter mergeToInClause(TupleFilter filter) {
			...
			CompareTupleFilter inFilter = new CompareTupleFilter(
		...
		public TupleFilter visitInputRef(RexInputRef inputRef) {
			TblColRef column = inputRowType.getColumnByIndex(inputRef
					.getIndex());
	...
	public OLAPFilterRel(RelOptCluster cluster, RelTraitSet traits,
	...
	public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
		return new OLAPFilterRel(getCluster(), traitSet, sole(inputs),
				condition);
	}
	...
	public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
		...
		RexProgramBuilder programBuilder = new RexProgramBuilder(inputRowType,
				rexBuilder);
		programBuilder.addIdentity();
		programBuilder.addCondition(this.condition);
		RexProgram program = programBuilder.getProgram();

		EnumerableCalcRel enumCalcRel = new EnumerableCalcRel(getCluster(),
				getCluster().traitSetOf(EnumerableConvention.INSTANCE),
				getChild(), this.rowType, program,
