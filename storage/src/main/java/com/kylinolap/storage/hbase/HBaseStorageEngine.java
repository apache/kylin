...
public class HBaseStorageEngine implements IStorageEngine {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(HBaseStorageEngine.class);
	...
	public ITupleIterator search(Collection<TblColRef> dimensions,
		...
		Set<TblColRef> storageGroupBy = Sets.newHashSet(context
				.getMandatoryColumns());
		...
		// family:column
		List<RowValueDecoder> valueDecoders = translateAggregation(
				cubeDesc.getHBaseMapping(), metrics, scans, context);

		setThreshold(dimensions, valueDecoders, context);

		HConnection conn = HBaseConnection.get(context.getConnUrl());
		return new SerializedHBaseTupleIterator(conn, scans, cubeInstance,
				dimensions, filter, storageGroupBy, valueDecoders, context);
	}

	private void collectGroupBy(Collection<TblColRef> groups,
			Set<TblColRef> storageGroupBy) {
	...
	private void collectNonEvaluable(TupleFilter filter,
			Set<TblColRef> storageGroupBy) {
		if (filter == null)
			return;
	...
	private void collectColumnsRecursively(TupleFilter filter,
			Set<TblColRef> storageGroupBy) {
		if (filter instanceof ColumnTupleFilter) {
			collectGroupBy(((ColumnTupleFilter) filter).getColumn(),
					storageGroupBy);
		}
		for (TupleFilter child : filter.getChildren()) {
	...
	private TupleFilter translateDerived(TupleFilter filter,
			Set<TblColRef> storageGroupBy) {
		if (filter == null)
			return filter;

		if (filter instanceof CompareTupleFilter) {
			return translateDerivedInCompare((CompareTupleFilter) filter,
					storageGroupBy);
		}

		List<TupleFilter> children = (List<TupleFilter>) filter.getChildren();
		for (int i = 0; i < children.size(); i++) {
			TupleFilter translated = translateDerived(children.get(i),
					storageGroupBy);
			if (children.get(i) != translated) {
				if ((filter instanceof LogicalTupleFilter) == false)
					throw new IllegalStateException(
							"Cannot replace derived filter");
	...
	private TupleFilter translateDerivedInCompare(CompareTupleFilter compf,
		...
		CubeManager cubeMgr = CubeManager.getInstance(this.cubeInstance
				.getConfig());
		CubeSegment seg = cubeInstance.getLatestReadySegment();
		LookupStringTable lookup = cubeMgr.getLookupTable(seg,
				hostInfo.dimension);
		Pair<TupleFilter, Boolean> translated = DerivedFilterTranslator
				.translate(lookup, hostInfo, compf, colf, constf);
	...
	private void setThreshold(Collection<TblColRef> dimensions,
			List<RowValueDecoder> valueDecoders, StorageContext context) {
	...
	private List<RowValueDecoder> translateAggregation(
			HBaseMappingDesc hbaseMapping,
			Collection<FunctionDesc> aggregations, List<HBaseKeyRange> scans,
			StorageContext context) {
		Map<HBaseColumnDesc, RowValueDecoder> codecMap = Maps.newHashMap();
		for (FunctionDesc aggrFunc : aggregations) {
			Collection<HBaseColumnDesc> hbCols = hbaseMapping
					.findHBaseColumnByFunction(aggrFunc);
			if (hbCols.isEmpty()) {
				throw new IllegalStateException(
						"can't find HBaseColumnDesc for function "
			...
			for (HBaseColumnDesc hbCol : hbCols) {
				...
				// Cuboid
				if (measure.isHolisticCountDistinct()
						&& context.requireNoPostAggregation()) {
					logger.info("Holistic count distinct chosen for "
							+ aggrFunc);
					break;
				}
			}
	...
	private List<HBaseKeyRange> buildScanRanges(TupleFilter flatFilter,
		...
		for (CubeSegment cubeSeg : cubeInstance
			...
			List<HBaseKeyRange> scanRanges = Lists
					.newArrayListWithCapacity(orAndDimRanges.size());
			for (Collection<ColumnValueRange> andDimRanges : orAndDimRanges) {
				HBaseKeyRange rowKeyRange = new HBaseKeyRange(dimensionColumns,
						andDimRanges, cubeSeg, cubeDesc);
	...
	private List<Collection<ColumnValueRange>> translateToOrAndDimRanges(
			TupleFilter flatFilter, CubeSegment cubeSegment) {
		...
		for (TupleFilter andFilter : flatFilter.getChildren()) {
			if (andFilter.getOperator() != FilterOperatorEnum.AND) {
				throw new IllegalStateException(
						"Filter should be AND instead of " + andFilter);
			}

			Collection<ColumnValueRange> andRanges = translateToAndDimRanges(
					andFilter.getChildren(), cubeSegment);
	...
	private List<Collection<ColumnValueRange>> preprocessConstantConditions(
		...
		Iterator<Collection<ColumnValueRange>> iterator = orAndRanges
				.iterator();
	...
	private Collection<ColumnValueRange> translateToAndDimRanges(
			List<? extends TupleFilter> andFilters, CubeSegment cubeSegment) {
		...
		for (TupleFilter filter : andFilters) {
			...
			ColumnValueRange range = new ColumnValueRange(comp.getColumn(),
					comp.getValues(), comp.getOperator());
	...
	private void andMerge(ColumnValueRange range,
			Map<TblColRef, ColumnValueRange> rangeMap) {
	...
	private List<HBaseKeyRange> mergeOverlapRanges(List<HBaseKeyRange> keyRanges) {
		...
		for (int index = 0; index < keyRanges.size(); index++) {
			...
			if (Bytes.compareTo(maxStopKey, keyRange.getStartKey()) < 0) {
				...
				HBaseKeyRange mergedRange = mergeKeyRange(keyRanges,
						beginIndex, index - 1);
		...
		// merge last range
		HBaseKeyRange mergedRange = mergeKeyRange(keyRanges, beginIndex,
				keyRanges.size() - 1);
		mergedRanges.add(mergedRange);
		if (logger.isDebugEnabled()) {
	...
	private HBaseKeyRange mergeKeyRange(List<HBaseKeyRange> keyRanges,
			int from, int to) {
		...
		if (mergeSize > 1) {
			...
			List<Pair<byte[], byte[]>> newFuzzyKeys = new ArrayList<Pair<byte[], byte[]>>(
					mergeSize);
			List<Collection<ColumnValueRange>> newFlatOrAndFilter = Lists
					.newLinkedList();
			...
			for (int k = from; k <= to; k++) {
				...
				if (nextRange.getPartitionColumnStartDate() > 0
					...
					partitionColumnStartDate = nextRange
							.getPartitionColumnStartDate();
				}
				if (nextRange.getPartitionColumnEndDate() < Long.MAX_VALUE
						&& nextRange.getPartitionColumnEndDate() > partitionColumnEndDate) {
					partitionColumnEndDate = nextRange
							.getPartitionColumnEndDate();
				}
			}

			partitionColumnStartDate = (partitionColumnStartDate == Long.MAX_VALUE) ? 0
					: partitionColumnStartDate;
			partitionColumnEndDate = (partitionColumnEndDate == 0) ? Long.MAX_VALUE
					: partitionColumnEndDate;
			keyRange =

			new HBaseKeyRange(cubeSegment, cuboid, startKey, stopKey,
					newFuzzyKeys, newFlatOrAndFilter, partitionColumnStartDate,
					partitionColumnEndDate);
		}
		return keyRange;
	}
	...
	private List<HBaseKeyRange> mergeTooManyRanges(List<HBaseKeyRange> keyRanges) {
		...
		// range
		List<HBaseKeyRange> mergedRanges = new LinkedList<HBaseKeyRange>();
		HBaseKeyRange mergedRange = mergeKeyRange(keyRanges, 0,
				keyRanges.size() - 1);
		mergedRanges.add(mergedRange);
		return mergedRanges;
	}

	private void postProcessFilter(List<HBaseKeyRange> scans,
			TupleFilter flatFilter, StorageContext context) {
		...
		if (!scans.isEmpty()) {
			...
			if (!cuboid.requirePostAggregation()
					&& context.isExactAggregation() && flatFilter == null
					&& !context.hasSort()) {
