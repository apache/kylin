...
public class CuboidCLI {
	...
	public static void main(String[] args) throws IOException {
		MetadataManager metaMgr = MetadataManager.getInstance(KylinConfig
				.getInstanceFromEnv());
	...
	public static int simulateCuboidGeneration(CubeDesc cube) {
		...
		while (!cuboidQueue.isEmpty()) {
			...
			Collection<Long> spnanningCuboids = scheduler
					.getSpanningCuboid(cuboid);
			for (Long sc : spnanningCuboids) {
				boolean notfound = cuboidSet.add(sc);
				if (!notfound) {
					throw new IllegalStateException(
							"Find duplicate spanning cuboid " + sc
									+ " from cuboid " + cuboid);
				}
				cuboidQueue.push(sc);
			}
		...
		if (enumCuboids.equals(cuboidSet) == false) {
			throw new IllegalStateException("Expected cuboid set "
					+ enumCuboids + "; but actual cuboid set " + cuboidSet);
		}

		int mathCount = mathCalcCuboidCount(cube);
		if (mathCount != enumCuboids.size()) {
			throw new IllegalStateException("Math cuboid count " + mathCount
					+ ", but actual cuboid count " + enumCuboids.size());
		}
	...
	public static int mathCalcCuboidCount(CubeDesc cube) {
		...
		for (AggrGroupMask aggrGroupMask : cube.getRowkey().getAggrGroupMasks()) {
			lastAggrGroupCount = mathCalcCuboidCount_aggrGroup(cube,
					aggrGroupMask);
	...
	private static int mathCalcCuboidCount_aggrGroup(CubeDesc cube,
			AggrGroupMask aggrGroupMask) {
	...
	private static int mathCalcCuboidCount_combination(CubeDesc cube,
			long colMask) {
