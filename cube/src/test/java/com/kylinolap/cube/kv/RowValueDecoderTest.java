...
public class RowValueDecoderTest extends LocalFileMetadataTestCase {
	...
	public void testDecode() throws Exception {
		CubeDesc cubeDesc = CubeManager.getInstance(this.getTestConfig())
				.getCube("test_kylin_cube_with_slr_ready").getDescriptor();
		HBaseColumnDesc hbaseCol = cubeDesc.getHBaseMapping().getColumnFamily()[0]
				.getColumns()[0];
		...
		assertEquals("[PRICE, MIN_PRICE_, MAX_PRICE_, COUNT__]",
				measureNames.toString());
		assertEquals("[333.1234567, 333.1111111, 333.1999999, 2]",
				Arrays.toString(measureValues));
	}
