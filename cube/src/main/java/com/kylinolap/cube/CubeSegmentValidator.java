...
public class CubeSegmentValidator {
	...
	public static CubeSegmentValidator getCubeSegmentValidator(
			CubeBuildTypeEnum buildType, CubePartitionType partitionType) {
		switch (buildType) {
		case MERGE:
	...
	void validate(CubeInstance cubeInstance, List<CubeSegment> newSegments)
			throws CubeIntegrityException {
	}

	public class MergeOperationValidator extends CubeSegmentValidator {
		private void checkContingency(CubeInstance cubeInstance,
			...
			for (CubeSegment segment : cubeInstance.getSegments()) {
				if (segment.getDateRangeStart() == newSegment
						.getDateRangeStart()) {
					startSeg = segment;
				}
			...
			if (null == startSeg
					|| null == endSeg
					|| startSeg.getDateRangeStart() >= endSeg
							.getDateRangeStart()) {
				throw new CubeIntegrityException("Invalid date range.");
			}
		}

		private void checkLoopTableConsistency(CubeInstance cube,
				List<CubeSegment> newSegments) throws CubeIntegrityException {

			CubeSegment cubeSeg = newSegments.get(0);
			DictionaryManager dictMgr = DictionaryManager.getInstance(cube
					.getConfig());
			...
			for (DimensionDesc dim : cube.getDescriptor().getDimensions()) {
				for (TblColRef col : dim.getColumnRefs()) {
					...
					try {
						if (cubeSeg.getCubeDesc().getRowkey()
								.isUseDictionary(col)
								&& !cube.getDescriptor()
										.getFactTable()
										.equalsIgnoreCase(
			...
			for (TblColRef col : cols) {
				...
				for (CubeSegment segment : segmentList) {
					...
					if (temp == null) {
						throw new CubeIntegrityException(
								"Dictionary is null on column: " + col
										+ " Segment: " + segment);
					}
			...
			for (CubeSegment segment : segmentList) {
				if (firstSegment == null) {
					...
					if ((a.size() == b.size()) && a.containsAll(b)) {
						...
						throw new CubeIntegrityException(
								"Segments with different snapshots cannot be merged");
					}
				}
			}
		...
		@Override
		public void validate(CubeInstance cubeInstance,
				List<CubeSegment> newSegments) throws CubeIntegrityException {
	...
	public class IncrementalBuildOperationValidator extends
		...
		void checkContingency(CubeInstance cubeInstance,
				List<CubeSegment> newSegments) throws CubeIntegrityException {
			if (newSegments.size() != 1) {
				throw new CubeIntegrityException("Invalid date range.");
			}
			...
			for (CubeSegment segment : cubeInstance.getSegments()) {
				if (segment.getDateRangeStart() == newSegment
						.getDateRangeStart()) {
					if (segment.getDateRangeEnd() == newSegment
							.getDateRangeEnd()) {
			...
			if (!hasMatchSegment) {
				if (cubeInstance.getSegments().size() == 0) {
					if (cubeInstance.getDescriptor().getCubePartitionDesc()
							.getPartitionDateStart() != newSegment
							.getDateRangeStart()) {
						throw new CubeIntegrityException("Invalid start date.");
					}
				} else {
					CubeSegment lastSegment = cubeInstance.getSegments().get(
							cubeInstance.getSegments().size() - 1);
					if (newSegment.getDateRangeStart() != lastSegment
							.getDateRangeEnd()) {
						throw new CubeIntegrityException("Invalid start date.");
					}
				}
	...
	public class UpdateBuildOperationValidator extends CubeSegmentValidator {
		...
		void validate(CubeInstance cubeInstance, List<CubeSegment> newSegments)
			...
			if (newSegments.size() != 1 && newSegments.size() != 2) {
				throw new CubeIntegrityException(
						"Invalid new segment count, got " + newSegments.size());
			}
			...
			for (CubeSegment newSegment : newSegments) {
				if (null == previousSeg) {
					...
					if (previousSeg.getDateRangeEnd() != newSegment
							.getDateRangeStart()) {
						throw new CubeIntegrityException("Invalid date range.");
					}
				}
			...
			if (cubeInstance.getSegments().size() == 0) {
				if (cubeInstance.getDescriptor().getCubePartitionDesc()
						.getPartitionDateStart() != newSegments.get(0)
						.getDateRangeStart()) {
					throw new CubeIntegrityException("Invalid start date.");
				}
			} else {
				...
				for (CubeSegment segment : cubeInstance.getSegments()) {
					if (segment.getDateRangeStart() == startSegment
							.getDateRangeStart()) {
						matchSeg = segment;
					}
				}
				...
				if (newSegments.size() == 2
						&& newSegments.get(newSegments.size() - 1)
								.getDateRangeEnd() < matchSeg.getDateRangeEnd()) {
					throw new CubeIntegrityException("Invalid date range.");
				}
			}
		}
