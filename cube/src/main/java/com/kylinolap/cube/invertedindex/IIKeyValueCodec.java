...
public class IIKeyValueCodec {
	...
	private void collectKeyValues(
			TimeSlice slice,
			int col,
			CompressedValueContainer container,
			ArrayList<Pair<ImmutableBytesWritable, ImmutableBytesWritable>> result) {
		ImmutableBytesWritable key = encodeKey(slice.getTimeParititon(),
				slice.getSliceNo(), col, -1);
	...
	private void collectKeyValues(
		...
		for (int v = 0; v < values.size(); v++) {
			ImmutableBytesWritable key = encodeKey(slice.getTimeParititon(),
					slice.getSliceNo(), col, v);
			result.add(new Pair<ImmutableBytesWritable, ImmutableBytesWritable>(
					key, values.get(v)));
		}
	}

	ImmutableBytesWritable encodeKey(long timePartition, int sliceNo, int col,
			int colValue) {
	...
	int encodeKey(long timePartition, int sliceNo, int col, int colValue,
			byte[] buf, int offset) {
		int i = offset;
	...
	private static class Decoder implements Iterable<TimeSlice> {
		...
		private void makeNext() {
			...
			if (containers != null) {
				next = new TimeSlice(info, lastPartition, lastSliceNo,
						containers);
			}
