/*
 * OVERRIDE POINTS:
 * - getInSubqueryThreshold(), was hard coded to return 20, now Integer.MAX_VALUE
 */
...
public class SqlToRelConverter {
	...
	protected static final Logger SQL2REL_LOGGER = EigenbaseTrace
			.getSqlToRelTracer();
	 ...
	 * 
	 * @param alreadyConvertedNonCorrSubqs
	 *            the other map
	 */
	 ...
	 * 
	 * @param enabled
	 *            true for immediate conversion (the default); false to generate
	 *            logical TableAccessRel instances
	 */
	 ...
	 * 
	 * @param enabled
	 *            true to allow ValuesRel to be generated (the default); false
	 *            to force substitution of ProjectRel+OneRowRel instead
	 */
	...
	private void checkConvertedType(SqlNode query, RelNode result) {
		if (!query.isA(SqlKind.DML)) {
			...
			if (!checkConvertedRowType(query, convertedRowType)) {
				RelDataType validatedRowType = validator
						.getValidatedNodeType(query);
				validatedRowType = uniquifyFields(validatedRowType);
				throw Util
						.newInternal("Conversion to relational algebra failed to preserve "
								+ "datatypes:\n"
								+ "validated type:\n"
	...
	public RelNode trimUnusedFields(RelNode rootRel) {
		...
		if (isTrimUnusedFields()) {
			...
			if (dumpPlan) {
				SQL2REL_LOGGER.fine(RelOptUtil.dumpPlan(
						"Plan after trimming unused fields", rootRel, false,
	 ...
	 *            a JDBC result set; <code>false</code> if the query will be
	 *            part of a view.
	 */
	public RelNode convertQuery(SqlNode query, final boolean needsValidation,
			final boolean top) {
		if (needsValidation) {
		...
		if (dumpPlan) {
			SQL2REL_LOGGER.fine(RelOptUtil.dumpPlan(
					"Plan after converting SqlNode to RelNode", result, false,
					SqlExplainLevel.EXPPLAN_ATTRIBUTES));
		}
	...
	protected void convertSelectImpl(final Blackboard bb, SqlSelect select) {
		...
		gatherOrderExprs(bb, select, select.getOrderList(), orderExprList,
				collationList);
		final RelCollation collation = cluster.traitSetOf().canonize(
				RelCollationImpl.of(collationList));
	...
	private void distinctify(Blackboard bb, boolean checkForDupExprs) {
		...
		if (checkForDupExprs && (rel instanceof ProjectRel)) {
			...
			final List<RelDataTypeField> fields = rel.getRowType()
					.getFieldList();
			...
			rel = new ProjectRel(cluster, rel, Pair.left(newProjects),
					Pair.right(newProjects), ProjectRel.Flags.BOXED);
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				undoProjects.add(Pair.of(
						(RexNode) new RexInputRef(squished.get(origin), field
								.getType()), field.getName()));
			}

			rel = new ProjectRel(cluster, rel, Pair.left(undoProjects),
					Pair.right(undoProjects), ProjectRel.Flags.BOXED);
		...
		rel = createAggregate(bb,
				BitSets.range(rel.getRowType().getFieldCount()), aggCalls);
	 ...
	 *            Collation list
	 * @param orderExprList
	 *            Method populates this list with orderBy expressions not
	 ...
	 */
	protected void convertOrder(SqlSelect select, Blackboard bb,
			RelCollation collation, List<SqlNode> orderExprList,
		...
		bb.setRoot(
				new SortRel(cluster, cluster.traitSetOf(Convention.NONE,
						collation), bb.root, collation, offset == null ? null
						: convertExpression(offset), fetch == null ? null
						: convertExpression(fetch)), false);
		...
		if (orderExprList.size() > 0) {
			...
			bb.setRoot(
					new ProjectRel(cluster, cluster
							.traitSetOf(RelCollationImpl.PRESERVE), bb.root,
							exprs, cluster.getTypeFactory().createStructType(
									rowType.getFieldList().subList(0,
											fieldCount)),
							ProjectRelBase.Flags.BOXED), false);
		}
	}
			...
			if ((sqlCall.getOperator() == SqlStdOperatorTable.AND)
				...
				if (childSqlCall.getOperator() == SqlStdOperatorTable.AND) {
					...
					for (int i = 0; i < orOperands.length; i++) {
						orOperands[i] = SqlStdOperatorTable.NOT.createCall(
								SqlParserPos.ZERO, andOperands[i]);
					}
					for (int i = 0; i < orOperands.length; i++) {
					...
					for (int i = 0; i < andOperands.length; i++) {
						andOperands[i] = SqlStdOperatorTable.NOT.createCall(
								SqlParserPos.ZERO, orOperands[i]);
					}
					for (int i = 0; i < andOperands.length; i++) {
						andOperands[i] = pushDownNotForIn(andOperands[i]);
					}
					return SqlStdOperatorTable.AND.createCall(
							SqlParserPos.ZERO, andOperands[0], andOperands[1]);
					...
					SqlInOperator inOp = (SqlInOperator) childSqlCall
							.getOperator();
					if (inOp.isNotIn()) {
						return SqlStdOperatorTable.IN
								.createCall(SqlParserPos.ZERO, inOperands[0],
										inOperands[1]);
					} else {
	...
	private void convertWhere(final Blackboard bb, final SqlNode where) {
		...
		if (!convertedWhere.isAlwaysTrue()) {
			...
			Set<String> correlatedVariablesBefore = RelOptUtil
					.getVariablesUsed(inputRel);

			bb.setRoot(CalcRel.createFilter(bb.root, convertedWhere), false);
			Set<String> correlatedVariables = RelOptUtil
					.getVariablesUsed(bb.root);
	...
	private void substituteSubquery(Blackboard bb, SqlNode node) {
		...
		case IN:
			...
			if ((leftKeyNode instanceof SqlCall)
					&& (((SqlCall) leftKeyNode).getOperator() instanceof SqlRowOperator)) {
			...
			if (seek instanceof SqlNodeList) {
				...
				if (!seenNull && (valueList.size() < getInSubqueryThreshold())) {
					...
					RexNode expression = convertInToOr(bb, leftJoinKeysForIn,
							valueList, isNotIn);
			...
			//
			converted = convertExists(seek, true, false, subqueryNeedsOuterJoin
					|| isNotIn);
	 ...
	 * 
	 * @param call
	 *            the call that references the subquery
	 * @param bb
	 *            blackboard used to convert the subquery
	 * @param converted
	 ...
	 */
	private boolean convertNonCorrelatedSubq(SqlCall call, Blackboard bb,
			RelNode converted, boolean isExists) {
		if (subqueryConverter.canConvertSubquery()
			...
			if (constExpr == null) {
				constExpr = subqueryConverter.convertSubquery(call, this,
						isExists, isExplain);
			}
			if (constExpr != null) {
	...
	private RexNode convertInToOr(final Blackboard bb,
			final RexNode[] leftKeys, SqlNodeList valuesList, boolean isNotIn) {
		List<RexNode> comparisons = new ArrayList<RexNode>();
		for (SqlNode rightVals : valuesList) {
			RexNode rexComparison = null;
			if (leftKeys.length == 1) {
				rexComparison = rexBuilder.makeCall(SqlStdOperatorTable.EQUALS,
						leftKeys[0], bb.convertExpression(rightVals));
			} else {
				assert rightVals instanceof SqlCall;
				final SqlBasicCall call = (SqlBasicCall) rightVals;
				assert (call.getOperator() instanceof SqlRowOperator)
						&& call.getOperands().length == leftKeys.length;
				rexComparison = RexUtil.composeConjunction(rexBuilder, RexUtil
						.generate(leftKeys.length,
								new Function1<Integer, RexNode>() {
									public RexNode apply(Integer i) {
										return rexBuilder.makeCall(
												SqlStdOperatorTable.EQUALS,
												leftKeys[i],
												bb.convertExpression(call
														.getOperands()[i]));
									}
								}), false);
			}
		...
		RexNode result = RexUtil.composeDisjunction(rexBuilder, comparisons,
				true);
		assert result != null;
	 ...
	 */
	private RexNode createJoinConditionForIn(Blackboard bb,
			List<RexNode> leftKeys, RelNode rightRel) {
		List<RexNode> joinConditions = new ArrayList<RexNode>();

		// right fields appear after the LHS fields.
		int rightInputOffset = bb.root.getRowType().getFieldCount();

		List<RelDataTypeField> rightTypeFields = rightRel.getRowType()
				.getFieldList();
		...
		for (Ord<RexNode> key : Ord.zip(leftKeys)) {
			joinConditions.add(rexBuilder.makeCall(SqlStdOperatorTable.EQUALS,
					key.e, rexBuilder.makeInputRef(rightTypeFields.get(key.i)
							.getType(), rightInputOffset + key.i)));
		}
	 ...
	 * @param isExists
	 *            Whether is an EXISTS predicate
	 * @param needsOuterJoin
	 *            Whether an outer join is needed
	 * @return join expression
	 * @pre extraExpr == null || extraName != null
	 */
	private RelNode convertExists(SqlNode seek, boolean isIn, boolean isExists,
		...
		return RelOptUtil.createExistsPlan(cluster, seekRel, isIn, isExists,
				needsOuterJoin);
	}
	...
	private RelNode convertQueryOrInList(Blackboard bb, SqlNode seek) {
		...
		if (seek instanceof SqlNodeList) {
			return convertRowValues(bb, seek, ((SqlNodeList) seek).getList(),
					false, null);
		} else {
	...
	private RelNode convertRowValues(Blackboard bb, SqlNode rowList,
			Collection<SqlNode> rows, boolean allowLiteralsOnly,
		...
		if (targetRowType != null) {
			...
			rowType = SqlTypeUtil.promoteToRowType(typeFactory,
					validator.getValidatedNodeType(rowList), null);
		}
		...
		for (SqlNode node : rows) {
			...
			if (isRowConstructor(node)) {
				...
				for (SqlNode operand : call.operands) {
					RexLiteral rexLiteral = convertLiteralInValuesList(operand,
							bb, rowType, tuple.size());
				...
				RexLiteral rexLiteral = convertLiteralInValuesList(node, bb,
						rowType, 0);
				if ((rexLiteral != null) && shouldCreateValuesRel) {
				...
				// convert "1" to "row(1)"
				call = (SqlBasicCall) SqlStdOperatorTable.ROW.createCall(
	...
	private RexLiteral convertLiteralInValuesList(SqlNode sqlNode,
			Blackboard bb, RelDataType rowType, int iField) {
		if (!(sqlNode instanceof SqlLiteral)) {
			return null;
		}
		...
		RexNode literalExpr = exprConverter.convertLiteral(bb,
				(SqlLiteral) sqlNode);
		...
		if (SqlTypeUtil.isExactNumeric(type)) {
			BigDecimal roundedValue = NumberUtil.rescaleBigDecimal(
					(BigDecimal) value, type.getScale());
		...
		if ((value instanceof NlsString)
			...
			return rexBuilder.makeCharLiteral(new NlsString(Util.rpad(
					unpadded.getValue(), type.getPrecision()), unpadded
					.getCharsetName(), unpadded.getCollation()));
		}
		return literal;
	}
	 ...
	 * @param node
	 *            the SQL parse tree
	 * @param registerOnlyScalarSubqueries
	 *            if set to true and the parse tree corresponds to a variation
	 *            of a select node, only register it if it's a scalar subquery
	 */
			...
			if (node instanceof SqlCall) {
				...
				for (SqlNode operand : ((SqlCall) node).getOperandList()) {
					if (operand != null) {
						...
						findSubqueries(bb, operand, kind == SqlKind.IN
				...
				for (int i = 0; i < nodes.size(); i++) {
					...
					findSubqueries(bb, child, kind == SqlKind.IN
							|| registerOnlyScalarSubqueries);
				}
			}
	...
	public RexNode convertExpression(SqlNode node) {
		...
		Blackboard bb = createBlackboard(new ParameterScope(
				(SqlValidatorImpl) validator, nameToTypeMap), null);
	 ...
	 *            Expression to translate
	 * @param nameToNodeMap
	 *            map from String to {@link RexNode}; when an
	 *            {@link SqlIdentifier} is encountered, it is used as a key and
	 *            translated to the corresponding value from this map
	 * @return Converted expression
	 */
	public RexNode convertExpression(SqlNode node,
			Map<String, RexNode> nameToNodeMap) {
		...
		Blackboard bb = createBlackboard(new ParameterScope(
				(SqlValidatorImpl) validator, nameToTypeMap), nameToNodeMap);
		return bb.convertExpression(node);
	}
	 ...
	 * 
	 * <p>
	 * This method is an extension-point for derived classes can override. If
	...
	private RexNode convertOver(Blackboard bb, SqlNode node) {
		...
		final ImmutableList.Builder<RexNode> partitionKeys = ImmutableList
				.builder();
		...
		final ImmutableList.Builder<RexFieldCollation> orderKeys = ImmutableList
				.builder();
	 ...
	 * 
	 * @param bb
	 *            Scope within which to resolve identifiers
	 * @param from
	...
	protected void convertFrom(Blackboard bb, SqlNode from) {
		...
		case TABLESAMPLE:
			...
			if (sampleSpec instanceof SqlSampleSpec.SqlSubstitutionSampleSpec) {
				...
				throw Util.newInternal("unknown TABLESAMPLE type: "
						+ sampleSpec);
			}
			return;

		case IDENTIFIER:
			final SqlValidatorNamespace fromNamespace = validator.getNamespace(
					from).resolve();
			if (fromNamespace.getNode() != null) {
				convertFrom(bb, fromNamespace.getNode());
				return;
			}
			final String datasetName = datasetStack.isEmpty() ? null
					: datasetStack.peek();
			boolean[] usedDataset = { false };
			RelOptTable table = SqlValidatorUtil.getRelOptTable(fromNamespace,
					catalogReader, datasetName, usedDataset);
		...
		case JOIN:
			...
			final Blackboard fromBlackboard = createBlackboard(
					validator.getJoinScope(from), null);
			SqlNode left = join.getLeft();
			SqlNode right = join.getRight();
			final boolean isNatural = join.isNatural();
			final JoinType joinType = join.getJoinType();
			final Blackboard leftBlackboard = createBlackboard(Util.first(
					validator.getJoinScope(left),
					((DelegatingScope) bb.scope).getParent()), null);
			final Blackboard rightBlackboard = createBlackboard(Util.first(
			...
			final RelNode joinRel = createJoin(fromBlackboard, leftRel,
					rightRel, conditionExp, convertedJoinType);
		...
		case UNNEST:
			...
			final RelNode childRel = CalcRel.createProject(
			...
			UncollectRel uncollectRel = new UncollectRel(cluster,
					cluster.traitSetOf(Convention.NONE), childRel);
	...
	protected void convertCollectionTable(Blackboard bb, SqlCall call) {
		...
		TableFunctionRel callRel = new TableFunctionRel(cluster, inputs,
				rexCall, elementType, validator.getValidatedNodeType(call),
				columnMappings);
	...
	protected void afterTableFunction(SqlToRelConverter.Blackboard bb,
			SqlCall call, TableFunctionRel callRel) {
	}
	...
	protected RelNode createJoin(Blackboard bb, RelNode leftRel,
			RelNode rightRel, RexNode joinCond, JoinRelType joinType) {
		...
		if (correlatedVariables.size() > 0) {
			...
			for (String correlName : correlatedVariables) {
				...
				SqlValidatorNamespace foundNs = lookup.bb.scope.resolve(
						originalRelName, ancestorScopes, nsIndexes);
				...
				if (correlInCurrentScope) {
					...
					if (childNamespaceIndex > 0) {
						...
						List<SqlValidatorNamespace> children = ((ListScope) ancestorScope)
								.getChildren();

						for (int i = 0; i < childNamespaceIndex; i++) {
							SqlValidatorNamespace child = children.get(i);
							namespaceOffset += child.getRowType()
									.getFieldCount();
						}
					}

					RelDataTypeField field = catalogReader.field(
							foundNs.getRowType(), originalFieldName);
					int pos = namespaceOffset + field.getIndex();

					assert field.getType() == lookup.getFieldAccess(correlName)
							.getField().getType();
					...
					if (bb.mapRootRelToFieldProjection.containsKey(bb.root)) {
						...
						// keys.
						Map<Integer, Integer> exprProjection = bb.mapRootRelToFieldProjection
								.get(bb.root);
						...
						if (exprProjection.containsKey(pos)) {
							...
							// correl not grouped
							throw Util.newInternal("Identifier '"
									+ originalRelName + "." + originalFieldName
									+ "' is not a group expr");
						}
					}

					CorrelatorRel.Correlation newCorVar = new CorrelatorRel.Correlation(
			...
			if (!correlations.isEmpty()) {
				CorrelatorRel rel = new CorrelatorRel(rightRel.getCluster(),
						leftRel, rightRel, joinCond, correlations, joinType);
		...
		if (!containsGet(joinCond)) {
			joinCond = pushDownJoinConditions(joinCond, leftCount, rightCount,
					extraLeftExprs, extraRightExprs);
		}
		if (!extraLeftExprs.isEmpty()) {
			final List<RelDataTypeField> fields = leftRel.getRowType()
					.getFieldList();
			leftRel = CalcRel.createProject(leftRel,
					new AbstractList<Pair<RexNode, String>>() {
						...
						public Pair<RexNode, String> get(int index) {
							if (index < leftCount) {
								...
								return Pair.<RexNode, String> of(
										extraLeftExprs.get(index - leftCount),
										null);
							}
						}
					}, true);
		...
		if (!extraRightExprs.isEmpty()) {
			final List<RelDataTypeField> fields = rightRel.getRowType()
					.getFieldList();
			final int newLeftCount = leftCount + extraLeftExprs.size();
			rightRel = CalcRel.createProject(rightRel,
					new AbstractList<Pair<RexNode, String>>() {
						@Override
						public int size() {
	 ...
	 * "emp" that computes the expression "emp.deptno + 1". The resulting join
	 * condition is a simple combination of AND, equals, and input fields.
	 */
	private RexNode pushDownJoinConditions(RexNode node, int leftCount,
			int rightCount, List<RexNode> extraLeftExprs,
		...
		case EQUALS:
			...
			for (int i = 0; i < operands.size(); i++) {
				...
				final RexNode e = pushDownJoinConditions(operand, leftCount,
						rightCount, extraLeftExprs, extraRightExprs);
				final List<RexNode> remainingOperands = Util.skip(operands,
						i + 1);
	 ...
	 *         is empty
	 */
	private RexNode convertUsing(RelNode leftRel, RelNode rightRel,
		...
		for (String name : nameList) {
			...
			RexNode left = rexBuilder.makeInputRef(leftField.getType(),
					leftField.getIndex());
			final RelDataType rightRowType = rightRel.getRowType();
			RelDataTypeField rightField = catalogReader.field(rightRowType,
					name);
			RexNode right = rexBuilder.makeInputRef(rightField.getType(),
			...
			if (conditionExp == null) {
				...
				conditionExp = rexBuilder.makeCall(SqlStdOperatorTable.AND,
						conditionExp, equalsCall);
			}
		}
		return conditionExp;
	...
	protected final void createAggImpl(Blackboard bb,
			AggConverter aggConverter, SqlNodeList selectList,
		...
		for (SqlNode groupExpr : groupList) {
			++i;
			final SqlNode expandedGroupExpr = validator.expand(groupExpr,
					bb.scope);
			...
			if (expandedGroupExpr instanceof SqlIdentifier) {
				...
				int[] nsIndexes = { -1 };
				final SqlValidatorScope[] ancestorScopes = { null };
				SqlValidatorNamespace foundNs = bb.scope.resolve(
						originalRelName, ancestorScopes, nsIndexes);
				...
				if (childNamespaceIndex > 0) {
					...
					List<SqlValidatorNamespace> children = ((ListScope) ancestorScopes[0])
							.getChildren();

					for (int j = 0; j < childNamespaceIndex; j++) {
						namespaceOffset += children.get(j).getRowType()
								.getFieldCount();
					}
				}
		...
		try {
			...
			if (preExprs.size() == 0) {
				...
				preExprs = Collections.singletonList((RexNode) rexBuilder
			...
			bb.setRoot(
					CalcRel.createProject(inputRel, preExprs, preNames, true),
					false);
			...
			for (SqlNode groupItem : groupList) {
				bb.columnMonotonicities
						.add(bb.scope.getMonotonicity(groupItem));
			}
			...
			final SelectScope selectScope = SqlValidatorUtil
					.getEnclosingSelectScope(bb.scope);
			final SqlValidatorNamespace selectNamespace = validator
					.getNamespace(selectScope.getNode());
			final List<String> names = selectNamespace.getRowType()
					.getFieldNames();
			int sysFieldCount = selectList.size() - names.size();
			for (SqlNode expr : selectList) {
				selectExprs.add(bb.convertExpression(expr));
				selectNames.add(k < sysFieldCount ? validator.deriveAlias(expr,
						k++) : names.get(k++ - sysFieldCount));
			}
		...
		bb.setRoot(
				CalcRel.createProject(bb.root, selectExprs, selectNames, true),
				false);
	 ...
	 *            Blackboard
	 * @param groupSet
	 *            Bit set of ordinals of grouping columns
	 * @param aggCalls
	 *            Array of calls to aggregate functions
	 * @return AggregateRel
	 */
	protected RelNode createAggregate(Blackboard bb, BitSet groupSet,
	 ...
	 *            Scope within which to resolve identifiers
	 * @param select
	 *            Select clause. Never null, because we invent a dummy SELECT if
	 *            ORDER BY is applied to a set operation (UNION etc.)
	 * @param orderList
	 *            Order by clause, may be null
	 * @param extraOrderExprs
	 *            Sort expressions which are not in the select clause (output)
	 * @param collationList
	 *            List of collations (output)
	 */
	protected void gatherOrderExprs(Blackboard bb, SqlSelect select,
			SqlNodeList orderList, List<SqlNode> extraOrderExprs,
		...
		for (SqlNode orderItem : orderList) {
			collationList.add(convertOrderItem(select, orderItem,
					extraOrderExprs, RelFieldCollation.Direction.ASCENDING,
	...
	protected RelFieldCollation convertOrderItem(SqlSelect select,
		...
		case DESCENDING:
			return convertOrderItem(select, ((SqlCall) orderItem).operand(0),
					extraExprs, RelFieldCollation.Direction.DESCENDING,
					nullDirection);
		case NULLS_FIRST:
			return convertOrderItem(select, ((SqlCall) orderItem).operand(0),
					extraExprs, direction,
					RelFieldCollation.NullDirection.FIRST);
		case NULLS_LAST:
			return convertOrderItem(select, ((SqlCall) orderItem).operand(0),
					extraExprs, direction, RelFieldCollation.NullDirection.LAST);
		}
	...
	protected RelNode decorrelateQuery(RelNode rootRel) {
		RelDecorrelator decorrelator = new RelDecorrelator(rexBuilder,
				mapRefRelToCorVar, mapCorVarToCorRel, mapFieldAccessToCorVar);
		boolean dumpPlan = SQL2REL_LOGGER.isLoggable(Level.FINE);
		...
		if (dumpPlan) {
			SQL2REL_LOGGER.fine(RelOptUtil.dumpPlan(
					"Plan after removing CorrelatorRel", newRootRel, false,
					SqlExplainLevel.EXPPLAN_ATTRIBUTES));
		}
	...
	protected RelNode convertSetOp(SqlCall call) {
		...
		final RelNode right = convertQueryRecursive(call.operand(1), false,
				null);
		boolean all = false;
		...
		switch (call.getKind()) {
		case UNION:
			return new UnionRel(cluster, ImmutableList.of(left, right), all);
		...
		case INTERSECT:
			...
			if (!all) {
				...
				throw Util
						.newInternal("set operator INTERSECT ALL not suported");
			}

		case EXCEPT:
			// TODO: all
			if (!all) {
				return new MinusRel(cluster, ImmutableList.of(left, right), all);
			} else {
	...
	protected RelNode convertInsert(SqlInsert call) {
		...
		if (modifiableTable != null) {
			return modifiableTable.toModificationRel(cluster, targetTable,
					catalogReader, massagedRel,
					TableModificationRel.Operation.INSERT, null, false);
		}
		return new TableModificationRel(cluster, targetTable, catalogReader,
	...
	private RelOptTable.ToRelContext createToRelContext() {
		return new RelOptTable.ToRelContext() {
			...
			public RelNode expandView(RelDataType rowType, String queryString,
				...
				return viewExpander
						.expandView(rowType, queryString, schemaPath);
			}
		};
	}
	...
	protected RelOptTable getTargetTable(SqlNode call) {
		...
		return SqlValidatorUtil.getRelOptTable(targetNs, catalogReader, null,
				null);
	}
	...
	protected RelNode convertColumnList(SqlInsert call, RelNode sourceRel) {
		...
		final RexNode sourceRef = rexBuilder.makeRangeReference(sourceRowType,
				0, false);
		...
		final List<RelDataTypeField> targetFields = targetRowType
				.getFieldList();
		final List<RexNode> sourceExps = new ArrayList<RexNode>(
		...
		for (int i = 0; i < targetFields.size(); ++i) {
			...
			sourceExps
					.set(i,
							castNullLiteralIfNeeded(sourceExps.get(i),
									field.getType()));
		}
	 ...
	 *            List of target column names, to be populated
	 * @param columnExprs
	 *            List of expressions, to be populated
	 */
	protected void collectInsertTargets(SqlInsert call,
			final RexNode sourceRef, final List<String> targetColumnNames,
	...
	private RelNode convertMerge(SqlMerge call) {
		...
		if (insertCall != null) {
			...
			// table
			level1InsertExprs = ((ProjectRel) insertRel.getInput(0))
					.getProjects();
			if (insertRel.getInput(0).getInput(0) instanceof ProjectRel) {
				level2InsertExprs = ((ProjectRel) insertRel.getInput(0)
						.getInput(0)).getProjects();
			}
		...
		for (int level1Idx = 0; level1Idx < nLevel1Exprs; level1Idx++) {
			if ((level2InsertExprs != null)
				...
				int level2Idx = ((RexInputRef) level1InsertExprs.get(level1Idx))
						.getIndex();
		...
		RelNode massagedRel = CalcRel.createProject(joinRel, projects, null,
				true);
	...
	private RexNode convertIdentifier(Blackboard bb, SqlIdentifier identifier) {
		...
		if (null != correlationName) {
			...
			final RexNode prev = bb.mapCorrelateVariableToRexNode.put(
					correlationName, e);
			assert prev == null;
		}
		return e;
	...
	protected RexNode adjustInputRef(Blackboard bb, RexInputRef inputRef) {
		...
		if (field != null) {
			return rexBuilder
					.makeInputRef(field.getType(), inputRef.getIndex());
		}
		return inputRef;
	}
	...
	private RelNode convertMultisets(final List<SqlNode> operands, Blackboard bb) {
		...
		for (int i = 0; i < operands.size(); i++) {
			...
			final RelNode input;
			if (op == SqlStdOperatorTable.MULTISET_VALUE) {
				final SqlNodeList list = new SqlNodeList(call.getOperandList(),
						call.getParserPosition());
				// assert bb.scope instanceof SelectScope : bb.scope;
				CollectNamespace nss = (CollectNamespace) validator
						.getNamespace(call);
				Blackboard usedBb;
				if (null != nss) {
			...
			CollectRel collectRel = new CollectRel(cluster,
					cluster.traitSetOf(Convention.NONE), input,
		...
		for (int i = 0; i < joinList.size(); i++) {
			...
			if (o instanceof List) {
				...
				RelNode projRel = CalcRel.createProject(new OneRowRel(cluster),
						selectList, fieldNameList);

				Set<String> correlatedVariables = RelOptUtil
						.getVariablesUsed(projRel);
	...
	/**
	 * Factory method that creates a join. A subclass can override to use a
	 ...
	 */
	protected RelNode createJoin(RelNode left, RelNode right,
	...
	private void convertSelectList(Blackboard bb, SqlSelect select,
	 ...
	 * @param nameList
	 *            List of names, one per column
	 * @param aliasList
	 *            Collection of aliases that have been used already
	 * @param columnMonotonicityList
	 *            List of monotonicity, one per column
	 */
	protected void extraSelectItems(Blackboard bb, SqlSelect select,
			List<RexNode> exprList, List<String> nameList,
			Collection<String> aliasList,
			List<SqlMonotonicity> columnMonotonicityList) {
	}

	private String deriveAlias(final SqlNode node, Collection<String> aliases,
			final int ordinal) {
	 ...
	 *            Blackboard
	 * @param values
	 *            Call to SQL VALUES operator
	 * @param targetRowType
	 *            Target row type
	 */
	private void convertValuesImpl(Blackboard bb, SqlCall values,
			RelDataType targetRowType) {
		// Attempt direct conversion to ValuesRel; if that fails, deal with
		// fancy stuff like subqueries below.
		RelNode valuesRel = convertRowValues(bb, values,
				values.getOperandList(), true, targetRowType);
		if (valuesRel != null) {
		...
		for (SqlNode rowConstructor1 : values.getOperandList()) {
			...
			for (Ord<SqlNode> operand : Ord
					.zip(rowConstructor.getOperandList())) {
				exps.add(Pair.of(tmpBb.convertExpression(operand.e),
						validator.deriveAlias(operand.e, operand.i)));
			}
			RelNode in = (null == tmpBb.root) ? new OneRowRel(cluster)
					: tmpBb.root;
			unionRels.add(CalcRel.createProject(in, Pair.left(exps),
					Pair.right(exps), true));
		}
	...
	protected class Blackboard implements SqlRexContext, SqlVisitor<RexNode> {
		 ...
		 *            node, say
		 * @param nameToNodeMap
		 *            Map which translates the expression to map a given
		 *            parameter into, if translating expressions; null otherwise
		 */
		 ...
		 *         rchen 2006-08-17: temporarily translate select * from X where
		 *         a not in (select b form Y); to select X.* from X, (select
		 *         distinct b from Y) where not (a = b);
		 */
		public RexNode register(RelNode rel, JoinRelType joinType,
			...
			if (root == null) {
				...
				final int origLeftInputCount = root.getRowType()
						.getFieldCount();
				if (leftJoinKeysForIn != null) {
					...
					ProjectRel newLeftInput = (ProjectRel) CalcRel
							.createProject(root, newLeftInputExpr, null, true);
					...
					for (int i = 0; i < leftJoinKeysForIn.length; i++) {
						...
						newLeftJoinKeysForIn
								.add(rexBuilder.makeInputRef(newLeftInput
										.getProjects().get(x).getType(), x));
					}

					joinCond = createJoinConditionForIn(this,
							newLeftJoinKeysForIn, rel);
				}

				int leftFieldCount = root.getRowType().getFieldCount();
				final RelNode join = createJoin(this, root, rel, joinCond,
						joinType);
				...
				if ((leftJoinKeysForIn != null)
					...
					final int rexRangeRefLength = leftJoinKeysForIn.length
							+ rightFieldLength;
					...
					return rexBuilder.makeRangeReference(returnType,
							origLeftInputCount, false);
				} else {
					return rexBuilder.makeRangeReference(rel.getRowType(),
							leftFieldCount, joinType.generatesNullsOnRight());
				}
			}
		}
		 ...
		 * 
		 * @param root
		 *            New root relational expression
		 * @param leaf
		 *            Whether the relational expression is a leaf, that is,
		 *            derived from an atomic relational expression such as a
		 *            table name in the from clause, or the projection on top of
		 *            a select-subquery. In particular, relational expressions
		...
		private void setRoot(List<RelNode> inputs, RelNode root,
		...
		RexNode lookupExp(String name) {
			if (nameToNodeMap != null) {
				...
				if (node == null) {
					throw Util
							.newInternal("Unknown identifier '"
									+ name
			...
			int[] offsets = { -1 };
			final SqlValidatorScope[] ancestorScopes = { null };
			SqlValidatorNamespace foundNs = scope.resolve(name, ancestorScopes,
					offsets);
			if (foundNs == null) {
				return null;
			}
		...
		RexNode lookup(int offset, LookupContext lookupContext) {
			...
			return rexBuilder.makeRangeReference(pair.left.getRowType(),
					pair.right, false);
		}
		...
		public void flatten(List<RelNode> rels, int systemFieldCount,
				int[] start, List<Pair<RelNode, Integer>> relOffsetList) {
			for (RelNode rel : rels) {
				if (leaves.contains(rel)) {
					...
					flatten(rel.getInputs(), systemFieldCount, start,
							relOffsetList);
				}
			}
		}
		...
		public RexNode convertExpression(SqlNode expr) {
			...
			case SCALAR_QUERY:
				...
				fieldAccess = rexBuilder.makeFieldAccess(rex, rex.getType()
						.getFieldCount() - 1);
				...
				if (needTruthTest) {
					fieldAccess = rexBuilder.makeCall(
							SqlStdOperatorTable.IS_TRUE, fieldAccess);
				}
				return fieldAccess;
			...
			case IN:
				...
				if (rex instanceof RexRangeRef) {
					...
					isNotInFilter = ((SqlInOperator) ((SqlCall) expr)
							.getOperator()).isNotIn();
				...
				if (needTruthTest) {
					...
					rexNode = rexBuilder.makeFieldAccess(rex, rex.getType()
							.getFieldCount() - 1);
					if (!isNotInFilter) {
						rexNode = rexBuilder.makeCall(
								SqlStdOperatorTable.IS_TRUE, rexNode);
					} else {
		 ...
		 * 
		 * @param index
		 *            Position where new expression was inserted
		 * @param count
		 *            Number of new expressions inserted
		 */
		public void adjustSubqueries(final int index, final int count) {
			for (Map.Entry<SqlNode, RexNode> entry : mapSubqueryToExpr
					.entrySet()) {
					...
					public RexNode visitRangeRef(RexRangeRef rangeRef) {
						if (rangeRef.getOffset() >= index) {
							return rexBuilder.makeRangeReference(
									rangeRef.getType(), rangeRef.getOffset()
											+ count, false);
						} else {
							return rangeRef;
						}
	...
	private class NoOpSubqueryConverter implements SubqueryConverter {
		...
		public RexNode convertSubquery(SqlCall subquery,
				SqlToRelConverter parentConverter, boolean isExists,
				boolean isExplain) {
	 ...
	 * 
	 * <p>
	 * Consider the expression SELECT deptno, SUM(2 * sal) FROM emp GROUP BY
	 * deptno Then
	 * 
	 * <ul>
	...
	protected class AggConverter implements SqlVisitor<Void> {
		 ...
		 */
		private final SqlNodeList groupExprs = new SqlNodeList(
				SqlParserPos.ZERO);
		...
		public Void visit(SqlCall call) {
			if (call.getOperator().isAggregator()) {
				...
				List<RelDataType> argTypes = call.getOperator() instanceof SqlCountAggFunction ? new ArrayList<RelDataType>(
				...
				final Aggregation aggregation = (Aggregation) call
						.getOperator();
				...
				final AggregateCall aggCall = new AggregateCall(aggregation,
						distinct, args, type, nameMap.get(call.toString()));
				RexNode rex = rexBuilder.addAggCall(aggCall, groupExprs.size(),
						aggCalls, aggCallMapping, argTypes);
	...
	private static class LookupContext {
		 ...
		 * 
		 * @param bb
		 *            Context for translating this subquery
		 * @param rels
		 *            Relational expressions
		 * @param systemFieldCount
		 ...
		 * 
		 * @param offset
		 *            Offset of relational expression in FROM clause
	...
	private class HistogramShuttle extends RexShuttle {
		...
		public RexNode visitCall(RexCall call) {
			...
			SqlFunction histogramOp = !ENABLE_HISTOGRAM_AGG ? null
			...
			if (histogramOp != null) {
				...
				if (histogramType != type) {
					...
					exprs.set(
							...
							reinterpretCast ? rexBuilder.makeReinterpretCast(
									...
									rexBuilder.makeLiteral(false)) : rexBuilder
									.makeCast(histogramType, exprs.get(0)));
				}

				RexCallBinding bind = new RexCallBinding(
						rexBuilder.getTypeFactory(),
						SqlStdOperatorTable.HISTOGRAM_AGG, exprs);

				RexNode over = rexBuilder
						.makeOver(SqlStdOperatorTable.HISTOGRAM_AGG
								.inferReturnType(bind),
								SqlStdOperatorTable.HISTOGRAM_AGG, exprs,
								partitionKeys, orderKeys, lowerBound,
								upperBound, window.isRows(), window
										.isAllowPartial(), false);

				RexNode histogramCall = rexBuilder.makeCall(histogramType,
						histogramOp, ImmutableList.of(over));

				// If needed, post Cast result back to original
				// type.
				if (histogramType != type) {
					if (reinterpretCast) {
						histogramCall = rexBuilder.makeReinterpretCast(type,
								histogramCall, rexBuilder.makeLiteral(false));
					} else {
						histogramCall = rexBuilder
								.makeCast(type, histogramCall);
					}
				}
				...
				boolean needSum0 = aggOp == SqlStdOperatorTable.SUM
						&& type.isNullable();
				SqlAggFunction aggOpToUse = needSum0 ? SqlStdOperatorTable.SUM0
						: aggOp;
				return rexBuilder.makeOver(type, aggOpToUse, exprs,
						partitionKeys, orderKeys, lowerBound, upperBound,
						window.isRows(), window.isAllowPartial(), needSum0);
			}
		}
		 ...
		 * 
		 * <p>
		 * For example, <code>getHistogramOp({@link
		 * SqlStdOperatorTable#MIN}}</code> returns
