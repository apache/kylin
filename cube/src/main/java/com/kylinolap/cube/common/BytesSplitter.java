...
public class BytesSplitter {
	...
	public static List<String> splitToString(byte[] bytes, int offset,
			byte delimiter) {
	...
	public byte inferByteRowDelimiter(byte[] bytes, int byteLen,
			int expectedSplits) throws IOException {

		if (expectedSplits > this.splitBuffers.length)
			throw new IOException(
					"expectSplits can not be greater than split buffer size");
				...
				try {
					...
					if (actualSplits == expectedSplits) {
						if (!delimiterFound) {
							...
							throw new IOException(
									"Duplicate delimiter found, found delimiter is : "
											+ foundDelimiter
											+ " new delimiter is " + c);
						}
					}
	...
	public int detectDelim(Text value, int expectedParts) {
		for (int i = 0; i < COMMON_DELIMS.length; i++) {
			int nParts = split(value.getBytes(), value.getLength(),
					(byte) COMMON_DELIMS[i]);
			if (nParts == expectedParts)
				return COMMON_DELIMS[i];
		}
		throw new RuntimeException(
				"Cannot detect delimeter from first line -- "
						+ value.toString() + " -- expect " + expectedParts
						+ " columns");
	}
