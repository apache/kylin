...
public class JobFetcher implements Job {
	...
	public void execute(JobExecutionContext context)
		...
		JobEngineConfig engineConfig = (JobEngineConfig) context.getJobDetail()
				.getJobDataMap().get(JobConstants.PROP_ENGINE_CONTEXT);
		...
		try {
			...
			List<JobInstance> pendingJobList = jobDAO
					.listAllJobs(JobStatusEnum.PENDING);
				...
				try {
					...
					for (String s : jobFlows.keySet()) {
						...
						if (cubename.equals(jobInstance.getRelatedCube())) {
							log.info("There is already a job of cube "
					...
					if (cubeHasRunningJob == false
							&& jobFlows.containsKey(JobInstance
									.getJobIdentity(jobInstance)) == false) {
						// create job flow
						JobFlow jobFlow = new JobFlow(jobInstance, engineConfig);
						jobFlows.put(JobInstance.getJobIdentity(jobInstance),
								jobFlow);
						...
						log.info("Job " + jobInstance.getUuid()
								+ " has been scheduled with the first step "
