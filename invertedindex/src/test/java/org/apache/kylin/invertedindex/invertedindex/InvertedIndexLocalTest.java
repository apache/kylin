...
public class InvertedIndexLocalTest extends LocalFileMetadataTestCase {
	...
	public void testCompressedValueContainer() {
		// create container
		CompressedValueContainer container = new CompressedValueContainer(
				info.getDigest(), 0, 500);
		Dictionary<String> dict = info.dict(0);
		...
		BytesUtil.writeUnsigned(Dictionary.NULL_ID[dict.getSizeOfId()], buf, 0,
				dict.getSizeOfId());
		container.append(bytes);
		container.closeForChange();
		...
		CompressedValueContainer container2 = new CompressedValueContainer(
				info.getDigest(), 0, 500);
		...
		for (int v = dict.getMinId(); v <= dict.getMaxId(); v++) {
			...
			int value = BytesUtil.readUnsigned(bytes.get(), bytes.getOffset(),
					bytes.getLength());
			assertEquals(v, value);
		}
		container2.getValueAt(i++, bytes);
	...
	public void testCodec() throws IOException {
		...
		assertEquals(new HashSet<TableRecord>(records),
				new HashSet<TableRecord>(recordsCopy));
		dump(recordsCopy);
	}
	...
	private List<Slice> buildTimeSlices(List<TableRecord> records)
			throws IOException {
