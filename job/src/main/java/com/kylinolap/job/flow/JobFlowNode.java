...
public class JobFlowNode implements InterruptableJob {
	...
	public void execute(JobExecutionContext context)
		...
		try {
			JobInstance jobInstance = updateJobStep(jobInstanceID, jobStepID,
					config, JobStepStatusEnum.RUNNING,
			...
			updateJobStep(jobInstanceID, jobStepID, config, output.getStatus(),
					null, System.currentTimeMillis(), output.getOutput());
			context.setResult(exitCode);

			log.info("Job status for " + context.getJobDetail().getKey()
					+ " has been updated.");
	...
	protected JobInstance updateJobStep(String jobInstanceUuid,
			int jobInstanceStepSeqId, KylinConfig config,
			JobStepStatusEnum newStatus, Long execStartTime, Long execEndTime,
		...
		try {
			...
			if (JobStepStatusEnum.WAITING == currentStatus
				...
				hasChange = true;
				currentStep
						.setExecWaitTime((System.currentTimeMillis() - currentStep
								.getExecStartTime()) / 1000);
			}
			if (null != newStatus) {
				hasChange = true;
		...
		if (null != execEndTime) {
			JobEngine.JOB_DURATION.put(
					...
					(double) (currentStep.getExecEndTime() - currentStep
							.getExecStartTime()) / 1000);
		}
	...
	protected void handleException(String jobInstanceUuid,
		...
		try {
			...
			String output = dao.getJobOutput(jobInstanceUuid,
					jobInstanceStepSeqId).getOutput();
			output = output + "\n" + exceptionMsg;
