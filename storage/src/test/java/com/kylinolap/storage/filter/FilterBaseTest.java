...
public class FilterBaseTest {
	...
	protected CompareTupleFilter buildCompareFilter(List<TblColRef> groups,
			int index) {
		TblColRef column = groups.get(index);
		CompareTupleFilter compareFilter = new CompareTupleFilter(
		...
		if (index == 0) {
			...
			constantFilter = new ConstantTupleFilter(
					"ClothinShoes & Accessories");
		}
	...
	protected TupleFilter buildAndFilter(List<TblColRef> columns) {
		...
		LogicalTupleFilter andFilter = new LogicalTupleFilter(
	...
	protected CompareTupleFilter buildCompareCaseFilter(List<TblColRef> groups,
			String constValue) {
		CompareTupleFilter compareFilter = new CompareTupleFilter(
				FilterOperatorEnum.EQ);
	...
	protected void compareFilter(TupleFilter f1, TupleFilter f2) {
		...
		if (s1 != s2) {
			throw new IllegalStateException("f1=" + str1 + ", f2=" + str2
					+ " has different children: " + s1 + " vs. " + s2);
		}
	...
	private static String[][] SAMPLE_DATA = new String[][] {
			{ "2013-03-10", "2012-01-12", "2014-03-10" },
			{ "ClothinShoes & Accessories", "ABIN", "FP-GTC", "FP-NON-GTC" } };

	protected Collection<Tuple> generateTuple(int number,
			List<TblColRef> columns, int[] matches) {
