...
public class TupleFilterSerializer {
	...
	private static void internalSerialize(TupleFilter filter,
			Decorator decorator, ByteBuffer buffer) {
	...
	private static void serializeFilter(int flag, TupleFilter filter,
			Decorator decorator, ByteBuffer buffer) {
		if (flag < 0) {
	...
	private static TupleFilter createTupleFilter(int opVal) {
		...
		case DYNAMIC:
			...
			break;
		default:
			throw new IllegalStateException("Error FilterOperatorEnum: "
					+ op.getValue());
		}
