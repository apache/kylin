...
public class Cuboid implements Comparable<Cuboid> {
	...
	public static boolean isValid(CubeDesc cube, long cuboidID) {
		...
		if (cuboidID < 0) {
			throw new IllegalArgumentException("Cuboid " + cuboidID
	...
	private static void addToQueue(Queue<Long> queue, List<Long> parents,
			HashSet<Long> dedupped) {
	...
	private static boolean checkBaseCuboid(RowKeyDesc rowkey, long cuboidID) {
		...
		if (cuboidID > baseCuboidId) {
			throw new IllegalArgumentException("Cubiod " + cuboidID
					+ " is out of scope 0-" + baseCuboidId);
		}
		return baseCuboidId == cuboidID;
	}

	private static boolean checkMandatoryColumns(RowKeyDesc rowkey,
			long cuboidID) {
	...
	private static boolean checkAggregationGroup(RowKeyDesc rowkey,
			long cuboidID) {
		long cuboidWithoutMandatory = cuboidID
				& ~rowkey.getMandatoryColumnMask();
		long leftover;
	...
	private boolean calcPostAggregation(long inputID, long id) {
		...
		List<HierarchyMask> hierarchyMaskList = cube.getRowkey()
				.getHierarchyMasks();
		if (hierarchyMaskList != null && hierarchyMaskList.size() > 0) {
