...
public class OLAPJoinRel extends EnumerableJoinRel implements OLAPRel {
	...
	public OLAPJoinRel(RelOptCluster cluster, RelTraitSet traits, RelNode left,
		...
		if (!remaining.isAlwaysTrue()) {
			logger.warn("OLAPJoinRel only supports equi-join, but " + condition
					+ " looks is not");
		}
	...
	private ColumnRowType buildColumnRowType() {
		...
		if (columns.size() != this.rowType.getFieldCount()) {
			throw new IllegalStateException("RowType="
					+ this.rowType.getFieldCount() + ", ColumnRowType="
					+ columns.size());
		}
	...
	private JoinDesc buildJoin(RexCall condition) {
		...
		for (Map.Entry<TblColRef, TblColRef> columnPair : joinColumns
				.entrySet()) {
	...
	private void translateJoinColumn(RexCall condition,
	...
	public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
		...
		if (this.hasSubQuery) {
			...
			result = implementor.result(physType, Blocks.toBlock(Expressions
					.call(factTable.getExpression(OLAPTable.class),
							"executeCubeQuery",
			...
			for (Map.Entry<String, RelDataType> rewriteField : this.context.rewriteFields
				...
				if (this.rowType.getField(fieldName, true) == null) {
					...
					RelDataTypeField newField = new RelDataTypeFieldImpl(
							fieldName, paramIndex++, fieldType);
