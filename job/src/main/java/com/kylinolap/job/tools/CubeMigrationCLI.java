...
public class CubeMigrationCLI {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(CubeMigrationCLI.class);
	...
	public static void moveCube(KylinConfig srcCfg, KylinConfig dstCfg,
			String cubeName, String projectName, String overwriteIfExists,
			String realExecute) throws IOException, JSONException,
			InterruptedException {
		...
		if (cube.getStatus() != CubeStatusEnum.READY)
			throw new IllegalStateException(
					"Cannot migrate cube that is not in READY state.");

		checkAndGetHbaseUrl();

		hbaseAdmin = new HBaseAdmin(HBaseConfiguration.create());
		hdfsFS = FileSystem.get(new Configuration());
		srcCoprocessorPath = DeployCoprocessorCLI.getNewestCoprocessorJar(
				srcConfig, hdfsFS);
		dstCoprocessorPath = DeployCoprocessorCLI.getNewestCoprocessorJar(
				dstConfig, hdfsFS);
	...
	public static void moveCube(String srcCfgUri, String dstCfgUri,
			String cubeName, String projectName, String overwriteIfExists,
			String realExecute) throws IOException, JSONException,
			InterruptedException {
		...
		moveCube(KylinConfig.createInstanceFromUri(srcCfgUri),
				...
				projectName, overwriteIfExists, realExecute);
	}
	...
	private static String checkAndGetHbaseUrl() {
		...
		if (srcIndex < 0 || dstIndex < 0)
			throw new IllegalStateException(
					"Both metadata urls should be hbase metadata url");
	...
	private static void renameTablesInHbase(CubeInstance cube) {
		...
		for (CubeSegment segment : cube.getSegments()) {
			String oldHTableName = segment.getStorageLocationIdentifier()
					.trim();
			String srcPrefix = CubeManager
					.getHbaseStorageLocationPrefix(srcConfig.getMetadataUrl());
			String dstPrefix = CubeManager
					.getHbaseStorageLocationPrefix(dstConfig.getMetadataUrl());

			logger.info("Hbase table path in the current segment: "
					+ oldHTableName);
			...
			if (oldHTableName.indexOf(srcPrefix) != 0)
				throw new IllegalStateException(
						"current hbase table is not starting with " + srcPrefix);

			String remaining = oldHTableName.substring(srcPrefix.length());
			String newHTableName = dstPrefix + remaining;

			operations.add(new Opt(OptType.RENAME_TABLE_IN_HBASE, new Object[] {
					oldHTableName, newHTableName }));
			operations.add(new Opt(OptType.ALTER_TABLE_COPROCESSOR,
					new Object[] { newHTableName }));
			renamedHTables.put(oldHTableName, newHTableName);
		}

		operations.add(new Opt(OptType.CHANGE_HTABLE_NAME_IN_CUBE,
				new Object[] { cube.getName(), renamedHTables }));
	}

	private static void copyFilesInMetaStore(CubeInstance cube,
			String overwriteIfExists) throws IOException {
		...
		for (String item : movingItems) {
			operations.add(new Opt(OptType.COPY_FILE_IN_META,
					new Object[] { item }));
		}
	}

	private static void addCubeIntoProject(String cubeName, String projectName)
			throws IOException {
		String projectResPath = ProjectInstance.concatResourcePath(projectName);
		if (!dstStore.exists(projectResPath))
			throw new IllegalStateException("The target project " + projectName
					+ "does not exist");
	...
	private static List<String> listCubeRelatedResources(CubeInstance cube)
			throws IOException {
	...
	private static void doOpt(Opt opt) throws IOException, InterruptedException {
		...
		case RENAME_TABLE_IN_HBASE: {
			...
			logger.info("Hbase table renamed from " + oldTableName + " to "
					+ newTableName);
			break;
		}
		case CHANGE_HTABLE_NAME_IN_CUBE: {
			...
			Serializer<CubeInstance> cubeSerializer = new JsonSerializer<CubeInstance>(
					CubeInstance.class);
		...
		case ADD_INTO_PROJECT: {
			...
			Serializer<ProjectInstance> projectSerializer = new JsonSerializer<ProjectInstance>(
		...
		case ALTER_TABLE_COPROCESSOR: {
			...
			DeployCoprocessorCLI.resetCoprocessor(htableName, hbaseAdmin,
					dstCoprocessorPath);
			logger.info("The hbase table " + htableName
					+ " is bound with new coprocessor " + dstCoprocessorPath);
			break;
		}
		}
	}
	...
	private static void undo(Opt opt) throws IOException, InterruptedException {
		...
		case RENAME_FOLDER_IN_HDFS: {
			...
			if (hdfsFS.exists(new Path(srcPath))
					&& !hdfsFS.exists(new Path(dstPath))) {
				hdfsFS.rename(new Path(srcPath), new Path(dstPath));
				logger.info("HDFS Folder renamed from " + srcPath + " to "
						+ dstPath);
			}
		...
		case RENAME_TABLE_IN_HBASE: {
			...
			if (hbaseAdmin.tableExists(oldTableName)
					&& !hbaseAdmin.tableExists(newTableName)) {
				String snapshotName = "_snapshot_" + oldTableName;
				hbaseAdmin.disableTable(oldTableName);
				hbaseAdmin.snapshot(snapshotName, oldTableName);
				hbaseAdmin.cloneSnapshot(snapshotName, newTableName);
		...
		case ALTER_TABLE_COPROCESSOR: {
			...
			DeployCoprocessorCLI.resetCoprocessor(htableName, hbaseAdmin,
					srcCoprocessorPath);
			logger.info("The hbase table " + htableName
					+ " is bound with new coprocessor " + srcCoprocessorPath);
			break;
		}
		}
	}
