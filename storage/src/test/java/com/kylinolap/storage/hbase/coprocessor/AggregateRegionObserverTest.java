...
public class AggregateRegionObserverTest {
	...
	HCol c1 = new HCol(family, q1, new String[] { "SUM", "COUNT" },
			new String[] { "decimal", "long" });
	HCol c2 = new HCol(family, q2, new String[] { "SUM" },
			new String[] { "decimal" });
	...
	public void test() throws IOException {
		...
		RegionScanner aggrScanner = new AggregationScanner(rowType, filter,
				projector, aggregators, innerScanner);
		...
		while (hasMore) {
			...
			hcol.measureCodec.decode(ByteBuffer.wrap(cell.getValueArray(),
					cell.getValueOffset(), cell.getValueLength()),
					hcol.measureValues);

			String rowKey = toString(cell.getRowArray(), cell.getRowOffset(),
					cell.getRowLength(), mask);
			String col = Bytes.toString(hcol.family) + ":"
					+ Bytes.toString(hcol.qualifier);
			...
			assertTrue(expectedResult.contains(rowKey + ", " + col + ", "
					+ values));
		}
		aggrScanner.close();
	}
	...
	private SRowType newRowType() {
		...
		t.setName("TABLE");
		TblColRef[] cols = new TblColRef[] { newCol("A", t), newCol("B", t),
				newCol("C", t), newCol("D", t) };
