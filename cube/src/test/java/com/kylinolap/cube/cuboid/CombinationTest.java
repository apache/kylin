...
public class CombinationTest {
	...
	public int findSmallerSibling(long valueBits, long valueMask) {
		if ((valueBits | valueMask) != valueMask) {
			throw new IllegalArgumentException("Dismatch "
					+ Long.toBinaryString(valueBits) + " from "
	...
	private int combination(long valueBits, long[] bitMasks, int offset,
			long prefix, int k) {
		if (k == 0) {
			if (prefix < valueBits) {
			...
			for (int i = offset; i < bitMasks.length; i++) {
				...
				if (newPrefix < valueBits) {
					count += combination(valueBits, bitMasks, i + 1, newPrefix,
							k - 1);
				}
			}
			return count;
	...
	public void testComb3() {
		...
		System.out.println("value = " + Long.toBinaryString(valueBits)
				+ ", count = " + Long.bitCount(valueBits));
		long valueMask = (long) Math.pow(2, 10) - 1;
		System.out.println("mask = " + Long.toBinaryString(valueMask)
				+ ", count = " + Long.bitCount(valueMask));
		System.out.println("************");
		int count = findSmallerSibling(valueBits, valueMask);
		System.out.println("smaller sibling count = " + count);
		int cnk = (int) calculateCombination(Long.bitCount(valueMask),
