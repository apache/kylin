...
public class StorageCleanupJob extends AbstractHadoopJob {
	...
	private static final Option OPTION_DELETE = OptionBuilder
			.withArgName("delete").hasArg().isRequired(false)
			.withDescription("Delete the unused storage").create("delete");

	protected static final Logger log = LoggerFactory
			.getLogger(StorageCleanupJob.class);
	...
	private boolean isJobInUse(JobInstance job) {
		if (job.getStatus().equals(JobStatusEnum.NEW)
				|| job.getStatus().equals(JobStatusEnum.PENDING)
	...
	private void cleanUnusedHBaseTables(Configuration conf)
		...
		for (CubeInstance cube : cubeMgr.listAllCubes()) {
			for (CubeSegment seg : cube.getSegments()) {
				...
				log.info("Remove table " + tablename
						+ " from drop list, as the table belongs to cube "
	...
	private void cleanUnusedHdfsFiles(Configuration conf) throws IOException {
		JobEngineConfig engineConfig = new JobEngineConfig(
		...
		// + "/kylin-.*");
		FileStatus[] fStatus = fs.listStatus(new Path(KylinConfig
		...
		List<JobInstance> allJobs = JobDAO.getInstance(
				KylinConfig.getInstanceFromEnv()).listAllJobs();
		for (JobInstance jobInstance : allJobs) {
			...
			if (isJobInUse(jobInstance) == true) {
				String path = JobInstance.getJobWorkingDir(jobInstance,
						engineConfig);
				allHdfsPathsNeedToBeDeleted.remove(path);
				log.info("Remove " + path
						+ " from deletion list, as the path belongs to job "
						+ jobInstance.getUuid() + " with status "
		...
		if (delete == true) {
			...
			System.out
					.println("--------------- HDFS Path To Be Deleted ---------------");
			for (String hdfsPath : allHdfsPathsNeedToBeDeleted) {
	...
	private void cleanUnusedIntermediateHiveTable(Configuration conf)
			throws IOException {
