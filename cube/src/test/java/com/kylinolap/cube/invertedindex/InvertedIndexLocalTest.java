...
public class InvertedIndexLocalTest extends LocalFileMetadataTestCase {
	...
	public void testCompressedValueContainer() {
		// create container
		CompressedValueContainer container = new CompressedValueContainer(info,
				0, 500);
		Dictionary<String> dict = info.dict(0);
		for (int v = dict.getMinId(); v <= dict.getMaxId(); v++) {
			container.append(v);
	...
	private List<TimeSlice> buildTimeSlices(List<TableRecord> records)
			throws IOException {
	...
	private List<Pair<ImmutableBytesWritable, ImmutableBytesWritable>> encodeKVs(
			IIKeyValueCodec codec, List<TimeSlice> slices) {
