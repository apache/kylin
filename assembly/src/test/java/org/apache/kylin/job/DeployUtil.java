...
import org.apache.kylin.source.hive.IJDBCExecutor;
...
public class DeployUtil {
    ...
    public static void prepareTestDataForNormalCubes(String modelName) throws Exception {
        ...
        if (!buildCubeUsingProvidedData) {
            ...
            ModelDataGenerator gen = new ModelDataGenerator(mgr, modelName, 10000);
            gen.generate();
        } else {
    ...
    private static void deployTables(String modelName) throws Exception {
        ...
        // create hive tables
        jdbcClient.executeHQL(jdbcClient.generateCreateSchemaSql("EDW"));
        for (String tablename : TABLE_NAMES) {
            logger.info(String.format("get table desc %s", tablename));
            jdbcClient.executeHQL(jdbcClient.generateCreateTableSql(metaMgr.getTableDesc(tablename)));
        }

        // load data to hive tables
        // LOAD DATA LOCAL INPATH 'filepath' [OVERWRITE] INTO TABLE tablename
        for (String tablename : TABLE_NAMES) {
            jdbcClient.executeHQL(jdbcClient.generateLoadDataSql(tablename.toUpperCase(), tempDirAbsPath));
        }
        ...
        jdbcClient.executeHQL(jdbcClient.generateCreateViewSql(VIEW_SELLER_TYPE_DIM, TABLE_SELLER_TYPE_DIM_TABLE));
        /*
