...
public class SqlToRelConverter {
    ...
    private static final Function<SubQuery, SqlNode> FN = new Function<SubQuery, SqlNode>() {
    ...
    private void distinctify(Blackboard bb, boolean checkForDupExprs) {
        ...
        // different.
        rel = createAggregate(bb, BitSets.range(rel.getRowType().getFieldCount()), ImmutableList.<AggregateCall> of());
    ...
    private void substituteSubquery(Blackboard bb, SubQuery subQuery) {
        ...
        case IN:
            ...
            if (converted.right) {
                ...
                AggregateRel aggregate = new AggregateRel(cluster, seek, BitSets.of(), ImmutableList.of(new AggregateCall(SqlStdOperatorTable.COUNT, false, ImmutableList.<Integer> of(), longType, null), new AggregateCall(SqlStdOperatorTable.COUNT, false, args, longType, null)));
                JoinRel join = new JoinRel(cluster, bb.root, aggregate, rexBuilder.makeLiteral(true), JoinRelType.INNER, ImmutableSet.<String> of());
    ...
    private RexNode convertInToOr(final Blackboard bb, final List<RexNode> leftKeys, SqlNodeList valuesList, boolean isNotIn) {
        ...
        for (SqlNode rightVals : valuesList) {
            ...
            if (leftKeys.size() == 1) {
                rexComparison = rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, leftKeys.get(0), bb.convertExpression(rightVals));
            } else {
                ...
                rexComparison = RexUtil.composeConjunction(rexBuilder, Iterables.transform(Pair.zip(leftKeys, call.getOperandList()), new Function<Pair<RexNode, SqlNode>, RexNode>() {
                    public RexNode apply(Pair<RexNode, SqlNode> pair) {
                        return rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, pair.left, bb.convertExpression(pair.right));
                    }
                }), false);
            }
    ...
    protected void convertFrom(Blackboard bb, SqlNode from) {
        ...
        case UNNEST:
            ...
            final RelNode childRel = RelOptUtil.createProject((null != bb.root) ? bb.root : new OneRowRel(cluster), Collections.singletonList(bb.convertExpression(node)), Collections.singletonList(validator.deriveAlias(node, 0)), true);
        ...
        if (correlatedVariables.size() > 0) {
            ...
            for (String correlName : correlatedVariables) {
                ...
                if (correlInCurrentScope) {
                    ...
                    Correlation newCorVar = new Correlation(getCorrelOrdinal(correlName), pos);
            ...
            if (!correlations.isEmpty()) {
                return new CorrelatorRel(rightRel.getCluster(), leftRel, rightRel, joinCond, correlations, joinType);
            }
        }
    ...
    protected final void createAggImpl(Blackboard bb, AggConverter aggConverter, SqlNodeList selectList, SqlNodeList groupList, SqlNode having, List<SqlNode> orderExprList) {
        ...
        bb.setRoot(RelOptUtil.createProject(bb.root, selectExprs, selectNames, true), false);
    ...
    protected class Blackboard implements SqlRexContext, SqlVisitor<RexNode> {
        ...
        private final Map<SqlNode, SubQuery> subqueryMap = Util.asIndexMap(subqueryList, FN);
        ...
        public RexNode register(RelNode rel, JoinRelType joinType, List<RexNode> leftKeys) {
            ...
            if (leftKeys != null) {
                ...
                ProjectRel newLeftInput = (ProjectRel) RelOptUtil.createProject(root, newLeftInputExpr, null, true);
