...
public class BitMapContainer implements ColumnValueContainer {
	...
	public void append(ImmutableBytesWritable valueBytes) {
		int value = BytesUtil.readUnsigned(valueBytes.get(),
				valueBytes.getOffset(), valueLen);
		append(value);
	}
	...
	public List<ImmutableBytesWritable> toBytes() {
		...
		List<ImmutableBytesWritable> r = new ArrayList<ImmutableBytesWritable>(
				nValues + 1);
		for (int i = 0; i <= nValues; i++) {
	...
	private ConciseSet bytesToSet(ImmutableBytesWritable bytes) {
		if (bytes.get() == null || bytes.getLength() == 0) {
			...
			IntBuffer intBuffer = ByteBuffer.wrap(bytes.get(),
					bytes.getOffset(), bytes.getLength()).asIntBuffer();
