...
public class HBaseKeyRange implements Comparable<HBaseKeyRange> {
	...
	private long calculateCuboidID(CubeDesc cube,
	...
	private void init(Collection<ColumnValueRange> andDimensionRanges) {
		...
		Map<TblColRef, String> startValues = Maps
				.newHashMapWithExpectedSize(size);
		Map<TblColRef, String> stopValues = Maps
				.newHashMapWithExpectedSize(size);
		Map<TblColRef, Set<String>> fuzzyValues = Maps
				.newHashMapWithExpectedSize(size);
		for (ColumnValueRange dimRange : andDimensionRanges) {
			...
			TblColRef partitionDateColumnRef = cubeSeg.getCubeDesc()
					.getCubePartitionDesc().getPartitionDateColumnRef();
			if (column.equals(partitionDateColumnRef)) {
				initPartitionRange(dimRange);
			}
		}

		AbstractRowKeyEncoder encoder = AbstractRowKeyEncoder.createInstance(
				cubeSeg, cuboid);
	...
	private void initPartitionRange(ColumnValueRange dimRange) {
		if (null != dimRange.getBeginValue()) {
			this.partitionColumnStartDate = DateStrDictionary.stringToDate(
					dimRange.getBeginValue()).getTime();
		}
		if (null != dimRange.getEndValue()) {
			this.partitionColumnEndDate = DateStrDictionary.stringToDate(
					dimRange.getEndValue()).getTime();
		}
	}
	...
	private List<Pair<byte[], byte[]>> buildFuzzyKeys(
			Map<TblColRef, Set<String>> fuzzyValueSet) {
		...
		List<Map<TblColRef, String>> fuzzyValues = FuzzyValueCombination
				.calculate(fuzzyValueSet, FUZZY_VALUE_CAP);
		for (Map<TblColRef, String> fuzzyValue : fuzzyValues) {
			result.add(new Pair<byte[], byte[]>(fuzzyKeyEncoder
					.encode(fuzzyValue), fuzzyMaskEncoder.encode(fuzzyValue)));
		}
		return result;
	}
	...
	public int hashCode() {
		...
		result = prime * result
				+ ((fuzzyKeyString == null) ? 0 : fuzzyKeyString.hashCode());
		result = prime * result
				+ ((startKeyString == null) ? 0 : startKeyString.hashCode());
		result = prime * result
				+ ((stopKeyString == null) ? 0 : stopKeyString.hashCode());
		return result;
	}
