...
@Slf4j
@Configuration
public class SmartAppInitializer {
    ...
    public void afterReady(ApplicationReadyEvent event) {
        ...
        if (config.isSmartNode() || config.isJobNode()) {
            // init QueryHistoryTaskScheduler
            QueryHistoryAccelerateScheduler.getInstance();

            // init check projects schedule
            topRecsUpdateScheduler.checkProject();
        }
    }
}
