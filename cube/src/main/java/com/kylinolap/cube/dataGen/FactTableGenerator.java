...
public class FactTableGenerator {
	...
	private void init(String cubeName, int rowCount, double conflictRaio,
			double linkableRatio, long randomSeed) {
	...
	private void loadLookupTableValues(String lookupTableName,
			LinkedList<String> columnNames, int distinctRowCount)
			throws Exception {
		KylinConfig config = KylinConfig.getInstanceFromEnv();

		// only deal with composite keys
		if (columnNames.size() > 1
				&& !lookupTableCompositeKeyValues.containsKey(lookupTableName)) {
			lookupTableCompositeKeyValues.put(lookupTableName,
					new HashSet<Array<String>>());
		}
		...
		try {
			...
			for (String columnName : columnNames) {
				ColumnDesc cDesc = MetadataManager.getInstance(config)
						.getTableDesc(lookupTableName)
			...
			distinctRowCount = (distinctRowCount < rowCount) ? distinctRowCount
					: rowCount;
			...
			while ((curRow = tableReader.readLine()) != null) {
				if (rows.contains(curRowNum)) {
					...
					for (String columnName : columnNames) {
						...
						if (!feasibleValues.containsKey(lookupTableName + "/"
								+ columnName))
							feasibleValues.put(lookupTableName + "/"
									+ columnName, new ArrayList<String>());
						feasibleValues.get(lookupTableName + "/" + columnName)
								.add(tokens[zeroBasedIndex]);
					...
					if (columnNames.size() > 1) {
						...
						if (lookupTableCompositeKeyValues.get(lookupTableName)
								.contains(wrap)) {
							throw new Exception(
									"The composite key already exist in the lookup table");
						}
						lookupTableCompositeKeyValues.get(lookupTableName).add(
								wrap);
					}
				}
				curRowNum++;
	...
	private void prepare() throws Exception {
			...
			if (join != null) {
				...
				for (String column : join.getPrimaryKey()) {
					if (!lookupTableKeys.containsKey(lookupTable)) {
						lookupTableKeys.put(lookupTable,
		...
		for (String lookupTable : lookupTableKeys.keySet()) {
			this.loadLookupTableValues(lookupTable,
					lookupTableKeys.get(lookupTable), distinctRowCount);
		}
	}
	...
	private List<DimensionDesc> getSortedDimentsionDescs() {
		...
		Collections.sort(dimensions, new Comparator<DimensionDesc>() {
			...
			public int compare(DimensionDesc o1, DimensionDesc o2) {
				...
				return new Integer(j1 != null ? j1.getPrimaryKey().length : 0)
						.compareTo(j2 != null ? j2.getPrimaryKey().length : 0);
			}
		});
		return dimensions;
	}
	...
	private void execute(String joinType) throws Exception {
		...
		String factTablePathWithJoinType = "/data/" + factTableName + ".csv."
				+ joinType.toLowerCase();
		store.deleteResource(factTablePathWithJoinType);
		store.putResource(factTablePathWithJoinType, in,
		...
		System.out.println();
		System.out
				.println("The new fact table has been written to $KYLIN_METADATA_URL"
						+ factTablePath);
		System.out.println();
	}
	...
	private String generate() throws Exception {
				...
				for (int i = 0; i < num; ++i) {
					...
					if (factTableCol2LookupCol.containsKey(fks[i])) {
						if (!factTableCol2LookupCol.get(fks[i]).equals(value)) {
							System.out
									.println("Warning: Disambiguation on the mapping of column "
											+ fks[i]
											+ ", "
		...
		for (MeasureDesc mDesc : desc.getMeasures()) {
			List<TblColRef> pcols = mDesc.getFunction().getParameter()
					.getColRefs();
			if (pcols != null) {
	...
	private String createRandomCell(ColumnDesc cDesc, ArrayList<String> range)
			throws Exception {
		...
		if (s.equals("string") || s.equals("char") || s.equals("varchar")) {
		...
		} else if (s.equals("bigint") || s.equals("int") || s.equals("tinyint")
				|| s.equals("smallint")) {
			...
			return cal.get(Calendar.YEAR) + "-"
					+ normToTwoDigits(cal.get(Calendar.MONTH) + 1) + "-"
	...
	private void printColumnMappings(
			HashMap<String, String> factTableCol2LookupCol,
		...
		System.out
				.println("You can vary the above parameters to generate different datasets.");
		System.out.println();
	}
	...
	private boolean matchAllCompositeKeys(
			HashMap<String, String> lookupCol2FactTableCol,
		...
		for (String lookupTable : lookupTableKeys.keySet()) {
			...
			String[] comboKey = new String[lookupTableKeys.get(lookupTable)
					.size()];
			int index = 0;
			for (String column : lookupTableKeys.get(lookupTable)) {
				...
				int cardinal = MetadataManager.getInstance(config)
						.getTableDesc(factTableName)
		...
		if ((cConfig = genConf.getColumnConfigByName(cDesc.getName())) == null) {
			...
			if (!cConfig.isAsRange() && !cConfig.isExclusive()
				...
				if (valueSet == null || valueSet.size() == 0)
					throw new Exception(
							"Did you forget to specify value set for "
									+ cDesc.getName());
				...
				if (!cConfig.isAsRange()) {
					...
					if (valueSet.size() != 2)
						throw new Exception(
								"Only two values can be set for range values, the column: "
										+ cDesc.getName());
	...
	private LinkedList<String> createRow(
			HashMap<String, String> factTableCol2LookupCol,
			HashSet<String> usedCols, HashSet<String> defaultColumns)
			throws Exception {
		KylinConfig config = KylinConfig.getInstanceFromEnv();
		...
		for (ColumnDesc cDesc : MetadataManager.getInstance(config)
				.getTableDesc(factTableName).getColumns()) {
			...
			if (factTableCol2LookupCol.containsKey(colName)) {
				...
				ArrayList<String> candidates = this.feasibleValues
						.get(factTableCol2LookupCol.get(colName));
				columnValues.add(candidates.get(Math.abs(seed)
						% candidates.size()));
	...
	private String createTable(int rowCount,
			HashMap<String, String> factTableCol2LookupCol,
			HashMap<String, String> lookupCol2FactTableCol,
			HashSet<String> usedCols) throws Exception {
		try {
			...
			for (int i = 0; i < rowCount;) {
				...
				LinkedList<String> columnValues = createRow(
						factTableCol2LookupCol, usedCols, defaultColumns);
	 ...
	 *            random seed
	 */
	public static void generate(String cubeName, String rowCount,
			String linkableRatio, String randomSeed, String joinType)
			throws Exception {
