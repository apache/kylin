...
public class QueryService extends BasicService {
	...
	public SQLResponse query(SQLRequest sqlRequest) throws Exception {
		SQLResponse fakeResponse = QueryUtil.tableauIntercept(sqlRequest
				.getSql());
	...
	public void saveQuery(final String name, final String project,
			final String sql, final String description) {
		final String creator = SecurityContextHolder.getContext()
	...
	public void removeQuery(final String id) {
		jdbcTemplate.update("DELETE FROM queries WHERE id = ?",
				new Object[] { id });
	}
	...
	public List<GeneralResponse> getQueries(final String creator) {
		...
		List<Map<String, Object>> rows = jdbcTemplate.queryForList(
				"SELECT * FROM queries WHERE creator = ?",
				new Object[] { creator });
	...
	public void logQuery(final SQLRequest request, final SQLResponse response,
		...
		if (!response.isHitCache()
				&& null != OLAPContext.getThreadLocalContexts()) {
			for (OLAPContext ctx : OLAPContext.getThreadLocalContexts()) {
				for (Cuboid cuboid : ctx.storageContext.getCuboids()) {
					String cubeName = cuboid.getCube().getName()
							.replace("_desc", "");
		...
		QueryMetrics.getInstance().increase("totalScanCount",
				(float) totalScanCount);
		...
		stringBuilder.append("Project: ").append(request.getProject())
				.append(newLine);
		...
		stringBuilder.append("Total scan count: ").append(totalScanCount)
				.append(newLine);
		stringBuilder.append("Result row count: ").append(resultRowCount)
				.append(newLine);
		stringBuilder.append("Accept Partial: ")
				.append(request.isAcceptPartial()).append(newLine);
		stringBuilder.append("Hit Cache: ").append(response.isHitCache())
				.append(newLine);
	...
	protected SQLResponse executeQuery(String sql, SQLRequest sqlRequest)
			throws Exception {
	...
	protected List<TableMeta> getMetadata(CubeManager cubeMgr, String project,
		...
		try {
			...
			ResultSet JDBCTableMeta = metaData
			...
			while (JDBCTableMeta.next()) {
				...
				TableMeta tblMeta = new TableMeta(
						catalogName == null ? Constant.FakeCatalogName
								: catalogName,
						schemaName == null ? Constant.FakeSchemaName
								: schemaName, JDBCTableMeta.getString(3),
						JDBCTableMeta.getString(4), JDBCTableMeta.getString(5),
						null, null, null, null, null);

				if (!cubedOnly
						|| getProjectManager().isExposedTable(project,
								tblMeta.getTABLE_NAME())) {
					tableMetas.add(tblMeta);
					tableMap.put(
							tblMeta.getTABLE_SCHEM() + "#"
									+ tblMeta.getTABLE_NAME(), tblMeta);
				}
			}
			...
			while (columnMeta.next()) {
				...
				ColumnMeta colmnMeta = new ColumnMeta(
						catalogName == null ? Constant.FakeCatalogName
								: catalogName,
						schemaName == null ? Constant.FakeSchemaName
								: schemaName, columnMeta.getString(3),
						columnMeta.getString(4), columnMeta.getInt(5),
						columnMeta.getString(6), columnMeta.getInt(7),
						getInt(columnMeta.getString(8)), columnMeta.getInt(9),
						columnMeta.getInt(10), columnMeta.getInt(11),
						columnMeta.getString(12), columnMeta.getString(13),
						getInt(columnMeta.getString(14)),
						getInt(columnMeta.getString(15)),
						columnMeta.getInt(16), columnMeta.getInt(17),
						columnMeta.getString(18), columnMeta.getString(19),
						columnMeta.getString(20), columnMeta.getString(21),
						getShort(columnMeta.getString(22)),
						columnMeta.getString(23));

				if (!cubedOnly
						|| getProjectManager().isExposedColumn(project,
								colmnMeta.getTABLE_NAME(),
								colmnMeta.getCOLUMN_NAME())) {
					tableMap.get(
							colmnMeta.getTABLE_SCHEM() + "#"
									+ colmnMeta.getTABLE_NAME()).addColumn(
							colmnMeta);
				}
			}
	 ...
	 * @throws Exception
	 */
	private SQLResponse execute(String sql, SQLRequest sqlRequest)
			throws Exception {
		Connection conn = null;
		Statement stat = null;
		...
		try {
			...
			if (sqlRequest instanceof PrepareSqlRequest) {
				...
				for (int i = 0; i < ((PrepareSqlRequest) sqlRequest)
						.getParams().length; i++) {
					setParam(preparedState, i + 1,
							((PrepareSqlRequest) sqlRequest).getParams()[i]);
				}
			...
			for (int i = 1; i <= columnCount; ++i) {
				columnMetas.add(new SelectedColumnMeta(metaData
						.isAutoIncrement(i), metaData.isCaseSensitive(i),
						metaData.isSearchable(i), metaData.isCurrency(i),
						metaData.isNullable(i), metaData.isSigned(i), metaData
								.getColumnDisplaySize(i), metaData
								.getColumnLabel(i), metaData.getColumnName(i),
						...
						metaData.isWritable(i), metaData
	...
	private void setParam(PreparedStatement preparedState, int index,
			StateParam param) throws SQLException {
		...
		case CHARACTER:
		case STRING:
			preparedState.setString(index,
					isNull ? null : String.valueOf(param.getValue()));
			break;
		case PRIMITIVE_INT:
		case INTEGER:
			preparedState.setInt(index,
					isNull ? null : Integer.valueOf(param.getValue()));
			break;
		case PRIMITIVE_SHORT:
		case SHORT:
			preparedState.setShort(index,
					isNull ? null : Short.valueOf(param.getValue()));
			break;
		case PRIMITIVE_LONG:
		case LONG:
			preparedState.setLong(index,
					isNull ? null : Long.valueOf(param.getValue()));
			break;
		case PRIMITIVE_FLOAT:
		case FLOAT:
			preparedState.setFloat(index,
					isNull ? null : Float.valueOf(param.getValue()));
			break;
		case PRIMITIVE_DOUBLE:
		case DOUBLE:
			preparedState.setDouble(index,
					isNull ? null : Double.valueOf(param.getValue()));
			break;
		case PRIMITIVE_BOOLEAN:
		case BOOLEAN:
			preparedState.setBoolean(index,
					isNull ? null : Boolean.parseBoolean(param.getValue()));
			break;
		case PRIMITIVE_BYTE:
		case BYTE:
			preparedState.setByte(index,
					isNull ? null : Byte.valueOf(param.getValue()));
			break;
		case JAVA_UTIL_DATE:
		case JAVA_SQL_DATE:
			preparedState.setDate(index,
					isNull ? null : java.sql.Date.valueOf(param.getValue()));
			break;
		case JAVA_SQL_TIME:
			preparedState.setTime(index,
					isNull ? null : Time.valueOf(param.getValue()));
			break;
		case JAVA_SQL_TIMESTAMP:
			preparedState.setTimestamp(index,
					isNull ? null : Timestamp.valueOf(param.getValue()));
			break;
		default:
