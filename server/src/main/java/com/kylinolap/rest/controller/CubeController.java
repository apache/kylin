...
public class CubeController extends BasicController {
	private static final Logger logger = LoggerFactory
			.getLogger(CubeController.class);
	...
	public List<CubeInstance> getCubes(
		...
		return cubeService.getCubes(cubeName, projectName, (null == limit) ? 20
				: limit, offset);
	}
	...
	public GeneralResponse getSql(@PathVariable String cubeName,
		...
		String sql = null;
		try {
			sql = cubeService.getJobManager().previewFlatHiveQL(cubeName,
					segmentName);
	...
	@ResponseBody
	public void updateNotifyList(@PathVariable String cubeName,
			@RequestBody List<String> notifyList) {
	...
	public CubeInstance updateCubeCost(@PathVariable String cubeName,
		...
		try {
			...
			String message = "Failed to update cube cost: " + cubeName + " : "
					+ cost;
			logger.error(message, e);
			throw new InternalErrorException(message + " Caused by: "
					+ e.getMessage(), e);
		}
	}
	...
	@ResponseBody
	public Map<String, Boolean> updateCubeCoprocessor(
		...
		try {
			...
			String message = "Failed to update cube coprocessor: " + cubeName
					+ " : " + force;
			logger.error(message, e);
			throw new InternalErrorException(message + " Caused by: "
					+ e.getMessage(), e);
	...
	public CubeInstance rebuildLookupSnapshot(@PathVariable String cubeName,
		...
		try {
			return cubeService.rebuildLookupSnapshot(cubeName, segmentName,
					lookupTable);
		} catch (IOException e) {
			logger.error(e.getLocalizedMessage(), e);
			throw new InternalErrorException(e.getLocalizedMessage());
	...
	@ResponseBody
	public JobInstance rebuild(@PathVariable String cubeName,
			@RequestBody JobBuildRequest jobBuildRequest) {
		JobInstance jobInstance = null;

		try {
			CubeInstance cube = jobService.getCubeManager().getCube(cubeName);
			String jobId = jobService.submitJob(cube,
					jobBuildRequest.getStartTime(),
					jobBuildRequest.getEndTime(),
	...
	public void deleteCube(@PathVariable String cubeName) {
		CubeInstance cube = cubeService.getCubeManager().getCube(cubeName);
		if (null == cube) {
			throw new NotFoundException("Cube with name " + cubeName
					+ " not found..");
		}
	...
	public CubeRequest updateCubeDesc(@RequestBody CubeRequest cubeRequest) {
		...
		if (!cubeService.isCubeDescEditable(desc)) {
			String error = "Cube desc " + desc.getName().toUpperCase()
					+ " is not editable.";
		...
		String descData = "";
		try {
			CubeInstance cube = cubeService.getCubeManager().getCube(
			...
			throw new ForbiddenException(
					"You don't have right to update this cube.");
		} catch (Exception e) {
			logger.error("Failed to deal with the request.", e);
	...
	private void updateRequest(CubeRequest request, boolean success,
			String message) {
