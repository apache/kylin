...
public class UuidLookupStrategy implements LookupStrategy {
	...
	private AclImpl convert(Map<Serializable, Acl> inputMap,
		...
		AclImpl result = new AclImpl(inputAcl.getObjectIdentity(),
				(Long) inputAcl.getId(), aclAuthorizationStrategy,
				grantingStrategy, parent, null, inputAcl.isEntriesInheriting(),
				inputAcl.getOwner());
		...
		public Set<Long> extractData(ResultSet rs) throws SQLException {
			...
			while (rs.next()) {
				...
				if (parentId != 0) {
					...
					MutableAcl cached = aclCache.getFromCache(Long
							.valueOf(parentId));
		...
		private void convertCurrentResultIntoObject(
			...
			if (acl == null) {
				...
				acl = new AclImpl(objectIdentity, id, aclAuthorizationStrategy,
						grantingStrategy, parentAcl, null, entriesInheriting,
						owner);
