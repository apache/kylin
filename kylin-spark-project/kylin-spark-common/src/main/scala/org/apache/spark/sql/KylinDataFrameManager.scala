...
class KylinDataFrameManager(sparkSession: SparkSession) {
  ...
  def cuboidTable(cubeInstance: CubeInstance, layout: Cuboid, segments: List[CubeSegment]): DataFrame = {
    option("project", cubeInstance.getProject)
    option("cubeId", cubeInstance.getUuid)
    option("cuboidId", layout.getId)
    val indexCatalog = new FilePruner(cubeInstance, layout, segments, sparkSession, options = extraOptions.toMap)
