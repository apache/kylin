...
public class HadoopStatusChecker {
	...
	protected static final Logger log = LoggerFactory
			.getLogger(HadoopStatusChecker.class);

	private final String yarnUrl;
	private final String mrJobID;
	private final StringBuilder output;

	public HadoopStatusChecker(String yarnUrl, String mrJobID,
	...
	public JobStepStatusEnum checkStatus() {
		...
		try {
			...
			RMAppState state = RMAppState.valueOf(root.findValue("state")
					.getTextValue());
			FinalApplicationStatus finalStatus = FinalApplicationStatus
					.valueOf(root.findValue("finalStatus").getTextValue());

			log.debug("State of Hadoop job: " + mrJobID + ":" + state + "-"
					+ finalStatus);
			output.append(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.S")
					.format(new Date())
					+ " - State of Hadoop job: "
					+ mrJobID
					+ ":" + state + " - " + finalStatus + "\n");
			...
			output.append("Failed to get status from response with url + "
					+ url + "\n");
