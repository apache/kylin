...
public class GenericJobEngineTest {
	...
	private static String mrCmd = "hadoop --config /etc/hadoop/conf jar /usr/lib/hadoop-mapreduce/hadoop-mapreduce-examples-2.*.jar wordcount "
			+ mrInputDir + " ";

	public static void removeHdfsDir(SSHClient hadoopCli, String hdfsDir)
			throws Exception {
	...
	public static void scpFilesToHdfs(SSHClient hadoopCli, String[] localFiles,
	...
	public static void beforeClass() throws Exception {
		...
		FileUtils.copyDirectory(new File("../examples/test_case_data"),
				new File(tempTestMetadataUrl));
		System.setProperty(KylinConfig.KYLIN_CONF, tempTestMetadataUrl);

		// deploy files to hdfs
		SSHClient hadoopCli = new SSHClient(getHadoopCliHostname(),
				getHadoopCliUsername(), getHadoopCliPassword(), null);
		scpFilesToHdfs(
				hadoopCli,
				new String[] { "src/test/resources/json/dummy_jobinstance.json" },
				mrInputDir);
		// deploy sample java jar
		hadoopCli.scpFileToRemote(
				"src/test/resources/jarfile/SampleJavaProgram.jarfile", "/tmp");
		hadoopCli.scpFileToRemote(
				"src/test/resources/jarfile/SampleBadJavaProgram.jarfile",
				"/tmp");
		...
		jobManager = new JobManager("GenericJobEngineTest",
	...
	public static void afterClass() throws Exception {
		...
		System.out.println("Max job step duration seconds: "
				+ jobManager.getMaxJobStepDuration());
		System.out.println("Min job step duration seconds: "
				+ jobManager.getMinJobStepDuration());
		System.out.println("# job steps executed: "
				+ jobManager.getNumberOfJobStepsExecuted());
	...
	public void testSubmitDuplicatedJobs() throws IOException,
			InvalidJobInstanceException, CubeIntegrityException {
	...
	public void testGoodJob() throws Exception {
		...
		CubeInstance cube = CubeManager.getInstance(
				KylinConfig.getInstanceFromEnv()).getCube(cubeName);
		String cubeString = JsonUtil.writeValueAsIndentString(cube);
		System.out.println(cubeString);
		assertEquals("good_job_uuid", cube.getSegments().get(0)
				.getLastBuildJobID());
	...
	public void testSingleStepBadJob() throws Exception {
		...
		String cubeString = JsonUtil.writeValueAsIndentString(cube);
		System.out.println(cubeString);
		assertEquals(CubeSegmentStatusEnum.NEW, cube.getSegments().get(0)
				.getStatus());
	}
	...
	public void testSelfCheckAndResume() throws Exception {
		...
		// check step status
		assertEquals(JobStepStatusEnum.FINISHED, jobManager.getJob(uuid)
				.getSteps().get(0).getStatus());
		assertEquals(JobStepStatusEnum.ERROR, jobManager.getJob(uuid)
				.getSteps().get(1).getStatus());
		assertEquals(JobStepStatusEnum.PENDING, jobManager.getJob(uuid)
				.getSteps().get(2).getStatus());

		jobManager.resumeJob(uuid);

		waitUntilJobComplete(uuid);
		assertEquals(JobStatusEnum.FINISHED, jobManager.getJob(uuid)
				.getStatus());
	}
	...
	public void testDiscardSyncStep() throws Exception {
		...
		assertEquals(JobStepStatusEnum.DISCARDED, killedJob.getSteps().get(0)
				.getStatus());
	...
	public void testKillMrStep() throws Exception {
		...
		JobInstance killedJob = jobManager.getJob(uuid);
		assertEquals(JobStepStatusEnum.ERROR, killedJob.getSteps().get(1)
				.getStatus());
	...
	private void waitUntilMrStepIsRunning(String jobUuid)
		...
		while (running == false) {
			...
			for (JobStep step : savedJob.getSteps()) {
				if (step.getCmdType().equals(
					...
					System.out.println("MR step is running with id "
							+ step.getInfo(JobInstance.MR_JOB_ID));
					running = true;
					break;
				}
			}
	...
	private void waitUntilJobComplete(String jobUuid) throws IOException,
	...
	private JobInstance createAGoodJobInstance(String uuid,
			int syncCmdSleepSeconds) throws IOException, CubeIntegrityException {
		CubeInstance cube = CubeManager.getInstance(
				KylinConfig.getInstanceFromEnv()).getCube(cubeName);
		cube.getSegments().clear();
		CubeManager.getInstance(KylinConfig.getInstanceFromEnv()).updateCube(
				cube);
		CubeSegment seg = CubeManager
				.getInstance(KylinConfig.getInstanceFromEnv())
				.allocateSegments(cube, CubeBuildTypeEnum.BUILD, 0, 12345L)
				.get(0);
		...
		step1.setName("step1");
		step1.setExecCmd("java -jar /tmp/SampleJavaProgram.jarfile "
				+ syncCmdSleepSeconds);
	...
	private JobInstance createASingleStepBadJobInstance(String uuid)
	...
	private static JobInstance createARunningJobInstance(String uuid)
			throws IOException, CubeIntegrityException {
		CubeInstance cube = CubeManager.getInstance(
				KylinConfig.getInstanceFromEnv()).getCube(cubeName);
		cube.getSegments().clear();
