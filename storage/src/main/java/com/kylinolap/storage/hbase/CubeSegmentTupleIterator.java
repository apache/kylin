...
public class CubeSegmentTupleIterator implements ITupleIterator {
	...
	public CubeSegmentTupleIterator(CubeSegment cubeSeg,
			Collection<HBaseKeyRange> keyRanges, HConnection conn,
	...
	private void closeScanner() {
		if (logger.isDebugEnabled() && scan != null) {
			...
			byte[] metricsBytes = scan
					.getAttribute(Scan.SCAN_ATTRIBUTES_METRICS_DATA);
			if (metricsBytes != null) {
				ScanMetrics scanMetrics = ProtobufUtil
						.toScanMetrics(metricsBytes);
				logger.debug(
						"HBase Metrics: "
								+ "count={}, ms={}, bytes={}, remote_bytes={}, regions={}, not_serving_region={}, rpc={}, rpc_retries={}, remote_rpc={}, remote_rpc_retries={}",
						new Object[] { scanCount,
								scanMetrics.sumOfMillisSecBetweenNexts,
								scanMetrics.countOfBytesInResults,
								scanMetrics.countOfBytesInRemoteResults,
								scanMetrics.countOfRegions,
								scanMetrics.countOfNSRE,
								scanMetrics.countOfRPCcalls,
								scanMetrics.countOfRPCRetries,
								scanMetrics.countOfRemoteRPCcalls,
		...
		try {
			...
			throw new StorageException("Error when close scanner for table "
					+ tableName, t);
		}
	}
	...
	public Tuple next() {
		...
		try {
			...
			throw new IllegalStateException("Can't translate result " + result,
					e);
		}
		return this.tuple;
	}
	...
	private Scan buildScan(HBaseKeyRange keyRange) {
		...
		for (RowValueDecoder valueDecoder : this.rowValueDecoders) {
			...
			byte[] byteFamily = Bytes
					.toBytes(hbaseColumn.getColumnFamilyName());
	...
	private TupleInfo buildTupleInfo(Cuboid cuboid) {
		...
		for (int i = 0; i < rowColumns.size(); i++) {
			...
			// add normal column
			info.setField(colNames.get(i), rowColumns.get(i), rowColumns.get(i)
					.getDatatype(), index++);
		}
		...
		for (Entry<Array<TblColRef>, List<DeriveInfo>> entry : hostToDerivedInfo
			...
			for (DeriveInfo deriveInfo : entry.getValue()) {
				...
				for (TblColRef derivedCol : deriveInfo.columns) {
					String derivedField = getFieldName(derivedCol,
							context.getAliasMap());
				...
				info.addDerivedColumnFiller(Tuple.newDerivedColumnFiller(
						...
						CubeManager.getInstance(this.cube.getConfig()), cubeSeg));
			}
		}
	...
	private void translateResult(Result res, Tuple tuple) throws IOException {
		...
		List<String> dimensionNames = rowKeyDecoder.getNames(context
				.getAliasMap());
