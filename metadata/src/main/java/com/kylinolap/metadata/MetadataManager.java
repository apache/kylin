...
public class MetadataManager {
	...
	private static final Serializer<CubeDesc> CUBE_SERIALIZER = new JsonSerializer<CubeDesc>(
			CubeDesc.class);
	private static final Serializer<TableDesc> TABLE_SERIALIZER = new JsonSerializer<TableDesc>(
			TableDesc.class);
	private static final Serializer<InvertedIndexDesc> IIDESC_SERIALIZER = new JsonSerializer<InvertedIndexDesc>(
			InvertedIndexDesc.class);
	...
	private SingleValueCache<String, InvertedIndexDesc> iiDescMap = new SingleValueCache<String, InvertedIndexDesc>(
			Broadcaster.TYPE.METADATA);
	// name => value
	private SingleValueCache<String, Map<String, String>> srcTableExdMap = new SingleValueCache<String, Map<String, String>>(
			Broadcaster.TYPE.METADATA);
	...
	public void createSourceTable(TableDesc srcTable) throws IOException {
		...
		if (srcTableMap.containsKey(srcTable.getName()))
			throw new IllegalArgumentException("SourceTable '"
					+ srcTable.getName() + "' already exists");
	...
	private void reloadAllSourceTableExd() throws IOException {
		...
		for (String path : paths) {
			...
			String tableName = loadSourceTableExd(getStore(), path,
					attrContainer);
	...
	public static String loadSourceTableExd(ResourceStore store, String path,
		...
		if (is != null) {
			...
			return file.substring(0, file.length()
					- MetadataConstances.FILE_SURFIX.length());
		} else {
	...
	private TableDesc loadSourceTable(String path) throws IOException {
		ResourceStore store = getStore();
		logger.debug("Loading SourceTable "
				+ store.getReadableResourcePath(path));

		TableDesc t = store
				.getResource(path, TableDesc.class, TABLE_SERIALIZER);
		t.init();

		if (StringUtils.isBlank(t.getName()))
			throw new IllegalStateException(
					"SourceTable name must not be blank");
		if (srcTableMap.containsKey(t.getName()))
			throw new IllegalStateException("Dup SourceTable name '"
					+ t.getName() + "'");
	...
	private void reloadAllCubeDesc() throws IOException {
		...
		for (String path : paths) {
			...
			if (cubeDescMap.containsKey(desc.getName())) {
				logger.error("Dup CubeDesc name '" + desc.getName()
						+ "' on path " + path);
				continue;
			}
	...
	private CubeDesc loadCubeDesc(String path) throws IOException {
		...
		if (ndesc.getError().isEmpty() == false) {
			throw new IllegalStateException("Cube desc at " + path
					+ " has issues: " + ndesc.getError());
		}
	...
	private void reloadAllInvertedIndexDesc() throws IOException {
		...
		List<String> paths = store.collectResourceRecursively(
				ResourceStore.IIDESC_RESOURCE_ROOT, ".json");
		for (String path : paths) {
			...
			if (iiDescMap.containsKey(desc.getName())) {
				logger.error("Dup InvertedIndexDesc name '" + desc.getName()
						+ "' on path " + path);
				continue;
			}
	...
	private InvertedIndexDesc loadInvertedIndexDesc(String path)
			throws IOException {
		ResourceStore store = getStore();
		logger.debug("Loading InvertedIndexDesc "
				+ store.getReadableResourcePath(path));

		InvertedIndexDesc desc = store.getResource(path,
				InvertedIndexDesc.class, IIDESC_SERIALIZER);
		if (StringUtils.isBlank(desc.getName())) {
			throw new IllegalStateException(
					"InvertedIndexDesc name must not be blank");
		}
