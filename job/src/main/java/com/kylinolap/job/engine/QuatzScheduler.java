...
public class QuatzScheduler {
	...
	public QuatzScheduler() throws JobException {
		this.globalJobListener = new JobFlowListener(
				JobConstants.GLOBAL_LISTENER_NAME);
		...
		schedulerProperties.setProperty("org.quartz.scheduler.skipUpdateCheck",
				"true");

		try {
			sf.initialize(schedulerProperties);
			this.scheduler = sf.getScheduler();
			this.scheduler.getListenerManager().addJobListener(
					this.globalJobListener,
					GroupMatcher
							.jobGroupEquals(JobConstants.CUBE_JOB_GROUP_NAME));
	...
	public void scheduleFetcher(int intervalInSeconds,
			JobEngineConfig engineConfig) throws JobException {
		JobDetail job = JobBuilder
				.newJob(JobFetcher.class)
				.withIdentity(JobFetcher.class.getCanonicalName(),
						JobConstants.DAEMON_JOB_GROUP_NAME).build();
	...
	public boolean interrupt(JobInstance jobInstance, JobStep jobStep)
			throws JobException, IOException {
		JobKey jobKey = new JobKey(JobInstance.getStepIdentity(jobInstance,
				jobStep), JobConstants.CUBE_JOB_GROUP_NAME);

		boolean res = false;
		try {
			JobDetail jobDetail = this.scheduler.getJobDetail(jobKey);

			IJobCommand iJobStepCmd = (IJobCommand) jobDetail.getJobDataMap()
					.get(JobConstants.PROP_JOB_CMD_EXECUTOR);
			if (null != iJobStepCmd) {
				iJobStepCmd.cancel();
			}
			...
			ConcurrentHashMap<String, JobFlow> jobFlows = (ConcurrentHashMap<String, JobFlow>) this.scheduler
					.getContext().get(JobConstants.PROP_JOB_RUNTIME_FLOWS);
	...
	public int getScheduledJobs() {
		...
		try {
			for (String groupName : scheduler.getJobGroupNames()) {
				allTriggersCount += scheduler.getJobKeys(
						GroupMatcher.jobGroupEquals(groupName)).size();
			}
		} catch (SchedulerException e) {
			log.error("Can't get scheduler metadata!", e);
		}
