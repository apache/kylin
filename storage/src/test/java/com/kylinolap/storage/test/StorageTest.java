...
public class StorageTest extends HBaseMetadataTestCase {
	...
	private int search(List<TblColRef> groups, List<FunctionDesc> aggregations,
			TupleFilter filter, StorageContext context) {
		int count = 0;
		ITupleIterator iterator = null;
		try {
			iterator = storageEngine.search(groups, filter, groups,
	...
	private CompareTupleFilter buildFilter1(TblColRef column) {
		...
		ConstantTupleFilter constantFilter1 = new ConstantTupleFilter(
				"2013-03-10");
	...
	private CompareTupleFilter buildFilter2(TblColRef column) {
		...
		ConstantTupleFilter constantFilter2 = new ConstantTupleFilter(
