...
public class KylinQueryTest extends KylinTestBase {
	...
	private static void setUpCubeConn() throws SQLException {
		// Cube Connection
		File olapTmp = OLAPSchemaFactory.createTempOLAPJson(
				ProjectInstance.DEFAULT_PROJECT_NAME, config);
		Properties props = new Properties();
		props.setProperty(OLAPQuery.PROP_SCAN_THRESHOLD, "10000");
		cubeConnection = DriverManager.getConnection("jdbc:optiq:model="
	...
	protected static void preferCubeOf(String joinType) {
		...
		boolean cubesBuiltInBatch = cubeManager
				.getCube("test_kylin_cube_with_slr_empty") != null
				&& cubeManager.getCube("test_kylin_cube_without_slr_empty") != null
				&& cubeManager
						.getCube("test_kylin_cube_with_slr_left_join_empty") != null
				&& cubeManager
						.getCube("test_kylin_cube_without_slr_left_join_empty") != null;
		...
		if (joinType.equals("inner")) {
			...
			cubeManager.getCube("test_kylin_cube_without_slr_empty")
					.setCost(10);
			cubeManager.getCube("test_kylin_cube_with_slr_left_join_empty")
					.setCost(100);
	...
	public void testInvalidQuery() throws Exception {
		...
		for (File sqlFile : sqlFiles) {
			...
			String sql = getTextFromFile(sqlFile);
			IDatabaseConnection cubeConn = new DatabaseConnection(
					cubeConnection);
			try {
