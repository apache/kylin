...
## 使用 memcached 作为 Kylin cache

从 v2.6.0，Kylin 可以使用 memcached 作为 cache。想要启用该功能，您需要执行以下步骤：

1. 在每一个节点上安装 memcached
...
- `kylin.query.cache-enabled` 的默认值为 true，故您可以不明确写出该参数。
- 如果您想要使用 lazy query cache，将 `kylin.query.lazy-query-enabled` 设置为 true。 
- `kylin.query.cache-signature-enabled` 设置为 true 将会开启 signature 检查。如果您的 signature 变化了，系统会移除老旧的 cache。cube 的 signature 取决于最新构建的时间。hybrid 的 signature 取决于 realization 实例的最新构建时间。SQLResponse 的 signature 取决于多个 realizations 的最新构建时间。 
- `kylin.query.segment-cache-enabled` 设置为 true 可以为那些频繁构建 cube 提升查询性能。您不能同时将 `kylin.query.segment-cache-enabled`, `kylin.query.lazy-query-enabled` and `kylin.query.cache-enabled` 设置为 true。这将使得 segment cache 失效。如果您不想要在当前查询中使用 segment cache，设置这个参数 `DEBUG_TOGGLE_DISABLE_QUERY_SEGMENT_CACHE=true`。该参数要放在 request body 中：
...
- `kylin.query.lazy-query-enabled`，`kylin.query.cache-signature-enabled` 和 `kylin.query.segment-cache-enabled` 的默认值都是 false。
- `kylin.cache.memcached.hosts` 指明了 memcached 的机器。