...
public class JobInstanceBuilder {
	...
	private static Logger log = LoggerFactory
			.getLogger(JobInstanceBuilder.class);
	...
	private void init(JobInstance jobInstance) {
		...
		cube = CubeManager.getInstance(this.engineConfig.getConfig()).getCube(
				cubeName);
		...
		if (segmentName == null || segmentName.equals("")) {
			throw new IllegalArgumentException(
					"Cube segment name is null or empty!");
		}
		...
		this.jobWorkingDir = JobInstance.getJobWorkingDir(jobInstance,
				engineConfig);
	}

	private String appendMapReduceParameters(String cmd) throws IOException {
		StringBuffer buf = new StringBuffer(cmd);
		String jobConf = this.engineConfig.getHadoopJobConfFilePath(cube
				.getDescriptor().getCapacity());
	...
	private String appendExecCmdParameters(String cmd, String paraName,
			String paraValue) {
		StringBuffer buf = new StringBuffer(cmd);
	...
	private String getIntermediateHiveTablePath() {
		JoinedFlatTableDesc intermediateTableDesc = new JoinedFlatTableDesc(
				cube.getDescriptor(), this.cubeSegment);
		return JoinedFlatTable.getTableDir(intermediateTableDesc,
				jobWorkingDir, jobUUID);
	}

	private String[] getCuboidOutputPaths(String cubeName,
			int totalRowkeyColumnCount, int groupRowkeyColumnsCount) {
		...
		for (int i = 0; i <= groupRowkeyColumnsCount; i++) {
			...
			if (dimNum == totalRowkeyColumnCount) {
				paths[i] = jobWorkingDir + "/" + cubeName + "/cuboid/"
						+ "base_cuboid";
			} else {
				paths[i] = jobWorkingDir + "/" + cubeName + "/cuboid/" + dimNum
						+ "d_cuboid";
			}
		}
		return paths;
	...
	private void createMergeCubeSegmentsSteps(JobInstance jobInstance)
			throws IOException {

		if (cube.getMergingSegments() == null
				|| cube.getMergingSegments().size() < 2) {
			throw new IllegalArgumentException(
					"Merging segments count should be more than 2");
		}
		...
		for (int i = 0; i < cube.getMergingSegments().size(); i++) {
			...
			cuboidPaths[i] = JobInstance.getJobWorkingDir(
		...
		addRangeRowkeyDistributionStep(jobInstance, stepSeqNum, jobWorkingDir
				+ "/" + cubeName + "/merged_cuboid");
		stepSeqNum++;
		...
		addConvertCuboidToHfileStep(jobInstance, stepSeqNum, jobWorkingDir
				+ "/" + cubeName + "/merged_cuboid");
		stepSeqNum++;
	...
	private void createBuildCubeSegmentSteps(JobInstance jobInstance)
			throws IOException {
		...
		int stepSeqNum = 0;
		int groupRowkeyColumnsCount = cube.getDescriptor().getRowkey()
				.getNCuboidBuildLevels();
		int totalRowkeyColumnsCount = cube.getDescriptor().getRowkey()
				.getRowKeyColumns().length;

		String[] cuboidOutputTempPath = getCuboidOutputPaths(cubeName,
				totalRowkeyColumnsCount, groupRowkeyColumnsCount);

		if (this.engineConfig.isFlatTableByHive()) {
			// by default in here

			// flat hive table step
			addIntermediateHiveTableStep(jobInstance, stepSeqNum,
					cuboidOutputTempPath);
			stepSeqNum++;
		}

		// fact distinct columns step
		addFactDistinctColumnsStep(jobInstance, stepSeqNum,
				cuboidOutputTempPath);
		stepSeqNum++;
		...
		for (int i = 1; i <= groupRowkeyColumnsCount; i++) {
			...
			addNDimensionCuboidStep(jobInstance, stepSeqNum,
					cuboidOutputTempPath, dimNum, totalRowkeyColumnsCount);
			stepSeqNum++;
		}
	...
	private void addBuildDictionaryStep(JobInstance jobInstance, int stepSeqNum) {
		...
		cmd = appendExecCmdParameters(cmd, "input",
				getFactDistinctColumnsPath());
		...
		buildDictionaryStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADOOP_NO_MR_DICTIONARY);

		jobInstance.addStep(stepSeqNum, buildDictionaryStep);
	}

	private void addIntermediateHiveTableStep(JobInstance jobInstance,
			int stepSeqNum, String[] cuboidOutputTempPath) throws IOException {
		JoinedFlatTableDesc intermediateTableDesc = new JoinedFlatTableDesc(
				cube.getDescriptor(), this.cubeSegment);
		String dropTableHql = JoinedFlatTable.generateDropTableStatement(
				intermediateTableDesc, jobUUID);
		String createTableHql = JoinedFlatTable.generateCreateTableStatement(
				intermediateTableDesc, jobWorkingDir, jobUUID);
		String insertDataHql = JoinedFlatTable.generateInsertDataStatement(
				intermediateTableDesc, jobUUID, this.engineConfig);

		JobStep intermediateHiveTableStep = new JobStep();
		intermediateHiveTableStep
				.setName(JobConstants.STEP_NAME_CREATE_FLAT_HIVE_TABLE);
		...
		intermediateHiveTableStep
				.setCmdType(JobStepCmdTypeEnum.SHELL_CMD_HADOOP);

		jobInstance.addStep(stepSeqNum, intermediateHiveTableStep);
	}

	private void addFactDistinctColumnsStep(JobInstance jobInstance,
			int stepSeqNum, String[] cuboidOutputTempPath) throws IOException {
		// base cuboid job
		...
		factDistinctColumnsStep
				.setName(JobConstants.STEP_NAME_FACT_DISTINCT_COLUMNS);

		cmd = appendExecCmdParameters(cmd, "cubename", cubeName);
		cmd = appendExecCmdParameters(cmd, "input", inputLocation);
		cmd = appendExecCmdParameters(cmd, "output",
				getFactDistinctColumnsPath());
		cmd = appendExecCmdParameters(cmd, "jobname",
				"Kylin_Fact_Distinct_Columns_" + jobInstance.getRelatedCube()
						+ "_Step_" + stepSeqNum);
		...
		factDistinctColumnsStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADOOP_FACTDISTINCT);

		jobInstance.addStep(stepSeqNum, factDistinctColumnsStep);
	}

	private void addBaseCuboidStep(JobInstance jobInstance, int stepSeqNum,
		...
		if (this.engineConfig.isFlatTableByHive()) {
			...
			HiveTable factTableInHive = new HiveTable(
					MetadataManager.getInstance(this.engineConfig.getConfig()),
					cube.getDescriptor().getFactTable());
		...
		cmd = appendExecCmdParameters(cmd, "jobname",
				"Kylin_Base_Cuboid_Builder_" + jobInstance.getRelatedCube()
						+ "_Step_" + stepSeqNum);
		...
		baseCuboidStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADOOP_BASECUBOID);

		jobInstance.addStep(stepSeqNum, baseCuboidStep);
	}

	private void addNDimensionCuboidStep(JobInstance jobInstance,
			int stepSeqNum, String[] cuboidOutputTempPath, int dimNum,
			int totalRowkeyColumnCount) throws IOException {
		// ND cuboid job
		JobStep ndCuboidStep = new JobStep();

		ndCuboidStep.setName(JobConstants.STEP_NAME_BUILD_N_D_CUBOID + " : "
				+ dimNum + "-Dimension");
		String cmd = "";
		...
		cmd = appendExecCmdParameters(cmd, "jobname",
				"Kylin_ND-Cuboid_Builder_" + jobInstance.getRelatedCube()
						+ "_Step_" + stepSeqNum);
		cmd = appendExecCmdParameters(cmd, "level", ""
	...
	private void addRangeRowkeyDistributionStep(JobInstance jobInstance,
			int stepSeqNum, String inputPath) throws IOException {
		JobStep rowkeyDistributionStep = new JobStep();
		rowkeyDistributionStep
				.setName(JobConstants.STEP_NAME_GET_CUBOID_KEY_DISTRIBUTION);
		String cmd = "";

		cmd = appendMapReduceParameters(cmd);
		cmd = appendExecCmdParameters(cmd, "input", inputPath);
		cmd = appendExecCmdParameters(cmd, "output",
				getRowkeyDistributionOutputPath());
		cmd = appendExecCmdParameters(
				cmd,
				"jobname",
				"Kylin_Region_Splits_Calculator_"
						+ jobInstance.getRelatedCube() + "_Step_" + stepSeqNum);
		cmd = appendExecCmdParameters(cmd, "cubename", cubeName);
	...
	private void addMergeCuboidDataStep(JobInstance jobInstance,
			int stepSeqNum, String inputPath) throws IOException {
		...
		cmd = appendExecCmdParameters(cmd, "segmentname", segmentName);
		cmd = appendExecCmdParameters(cmd, "input", inputPath);
		cmd = appendExecCmdParameters(cmd, "output", jobWorkingDir + "/"
				+ cubeName + "/merged_cuboid");
		cmd = appendExecCmdParameters(cmd, "jobname", "Kylin_Merge_Cuboid_"
				+ jobInstance.getRelatedCube() + "_Step_" + stepSeqNum);
		...
		mergeCuboidDataStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADOOP_MERGECUBOID);
	...
	private void addCreateHTableStep(JobInstance jobInstance, int stepSeqNum) {
		...
		cmd = appendExecCmdParameters(cmd, "cubename", cubeName);
		cmd = appendExecCmdParameters(cmd, "input",
				getRowkeyDistributionOutputPath() + "/part-r-00000");
		...
		createHtableStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADDOP_NO_MR_CREATEHTABLE);

		jobInstance.addStep(stepSeqNum, createHtableStep);
	}

	private void addConvertCuboidToHfileStep(JobInstance jobInstance,
			int stepSeqNum, String inputPath) throws IOException {
		JobStep createHFilesStep = new JobStep();
		createHFilesStep
				.setName(JobConstants.STEP_NAME_CONVERT_CUBOID_TO_HFILE);
		String cmd = "";
		...
		cmd = appendExecCmdParameters(cmd, "output", jobWorkingDir + "/"
				+ cubeName + "/hfile");
		cmd = appendExecCmdParameters(cmd, "htablename", htablename);
		cmd = appendExecCmdParameters(cmd, "jobname", "Kylin_HFile_Generator_"
				+ jobInstance.getRelatedCube() + "_Step_" + stepSeqNum);
		...
		createHFilesStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADOOP_CONVERTHFILE);
	...
	private void addBulkLoadStep(JobInstance jobInstance, int stepSeqNum) {
		...
		String cmd = "";
		cmd = appendExecCmdParameters(cmd, "input", jobWorkingDir + "/"
				+ cubeName + "/hfile/");
		cmd = appendExecCmdParameters(cmd, "htablename", htablename);
		cmd = appendExecCmdParameters(cmd, "cubename", cubeName);
		...
		bulkLoadStep
				.setCmdType(JobStepCmdTypeEnum.JAVA_CMD_HADOOP_NO_MR_BULKLOAD);
