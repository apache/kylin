  ...
  <div id="streamingJobListPage">
    ...
    <div class="streaming-jobs-list ksd-mrl-24">
      ...
      <el-row :gutter="20" class="jobs_tools_row ksd-mtb-8">
        ...
        <el-col :span="8">
          <el-input :placeholder="$t('pleaseSearch')" v-model="filter.model_name" v-global-key-event.enter.debounce="filterChange" @clear="filterChange()" class="show-search-btn ksd-fright" size="medium" prefix-icon="el-icon-search">
        ...
        <el-col :span="6" v-if="showStep" id="rightDetail" :class="{'is-admin-tips': $store.state.user.isShowAdminTips&&isAdminRole}">
          <el-tabs v-model="jobDetailTab" @tab-click="handleChangeTab">
            <el-tab-pane :label="$t('dataStatistics')" name="statistics">
              ...
              <kylin-empty-data
                v-else
                :content="$t('isNoStatisticsData')"
                :showImage="false"></kylin-empty-data>
            </el-tab-pane>
            <el-tab-pane :label="$t('recordStatus')" name="records">
              ...
              <kylin-empty-data
                :showImage="false"
                v-else
                :content="$t('isNoRecords')"></kylin-empty-data>
            </el-tab-pane>
          </el-tabs>
...
export default class StreamingJobsList extends Vue {
  ...
  filterContent (val, type) {
    ...
    val.length && val.forEach(item => {
      if (!list.includes(item)) {
        ...
        this.filterTags.push({ label, source: maps[type], key: type })
      }
    })
  ...
  getJobsList () {
    return new Promise((resolve, reject) => {
      if (!this.currentSelectedProject) return reject(new Error('no project'))
      const data = {}
  ...
  isShowJobBtn (e) {
    ...
    const jobBtn = document.getElementById('jobDetailBtn')
    if (jobBtn) {
