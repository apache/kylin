...
import static com.kylinolap.dict.TrieDictionary.BIT_IS_END_OF_VALUE;
import static com.kylinolap.dict.TrieDictionary.BIT_IS_LAST_CHILD;
import static com.kylinolap.dict.TrieDictionary.HEAD_MAGIC;
import static com.kylinolap.dict.TrieDictionary.HEAD_SIZE_I;
...
public class TrieDictionaryBuilder<T> {
	...
	private void addValueR(Node node, byte[] value, int start) {
		...
		if (j == nn) {
			...
			// otherwise, split the current node into two
				...
				Node c = new Node(subarray(node.part, i, n), node.isEndOfValue,
						node.children);
		...
		// value, make a 3-way
		if (i < n) {
			Node c1 = new Node(subarray(node.part, i, n), node.isEndOfValue,
					node.children);
	...
	public static class Stats {
		...
		public void print() {
			...
			out.println("OBPN node size:  "
					+ (obpn_sizeValue + obpn_sizeNoValuesBeneath
							+ obpn_sizeChildCount + obpn_sizeChildOffset)
					+ " = " + obpn_sizeValue + " + " + obpn_sizeNoValuesBeneath
					+ " + " + obpn_sizeChildCount + " + "
			...
			out.println("MBPN average fan out:   " + (double) mbpn_nTotalFanOut
					/ mbpn_nChildLookups);
			...
			out.println("MBPN node size:         "
					...
					+ " = " + mbpn_sizeNoValueBytes + " + "
					+ mbpn_sizeNoValueBeneath + " + " + mbpn_sizeChildOffset);
			out.println("MBPN no. nodes:         " + mbpn_nNodes);
			out.println("MBPN trie depth:        " + mbpn_trieDepth);
			out.println("MBPN footprint:         " + mbpn_footprint
					+ " in bytes");
		}
	}
	...
	public Stats stats() {
		...
		traverse(new Visitor() {
			...
			public void visit(Node n, int level) {
				...
				if (n.children.size() > 0) {
					...
					int childLookups = n.nValuesBeneath
							- (n.isEndOfValue ? 1 : 0);
		...
		s.obpn_nNodes = s.nValueBytesCompressed; // no. nodes is the total
													...
													// bytes in OBPN
		s.obpn_footprint = s.obpn_nNodes
				* (s.obpn_sizeValue + s.obpn_sizeNoValuesBeneath
						+ s.obpn_sizeChildCount + s.obpn_sizeChildOffset);
		while (true) { // minimize the offset size to match the footprint
			int t = s.obpn_nNodes
					* (s.obpn_sizeValue + s.obpn_sizeNoValuesBeneath
							+ s.obpn_sizeChildCount + s.obpn_sizeChildOffset - 1);
		...
		s.mbpn_footprint = s.mbpn_sizeValueTotal
				+ s.mbpn_nNodes
				* (s.mbpn_sizeNoValueBytes + s.mbpn_sizeNoValueBeneath + s.mbpn_sizeChildOffset);
		while (true) { // minimize the offset size to match the footprint
			int t = s.mbpn_sizeValueTotal
					+ s.mbpn_nNodes
					* (s.mbpn_sizeNoValueBytes + s.mbpn_sizeNoValueBeneath
							+ s.mbpn_sizeChildOffset - 1);
	...
	protected byte[] buildTrieBytes(int baseId) {
		...
		try {
			...
			headOut.writeUTF(bytesConverter == null ? "" : bytesConverter
					.getClass().getName());
			headOut.close();
		...
		o = build_writeNode(root, o, true, sizeNoValuesBeneath,
				sizeChildOffset, trieBytes);
		if (root.children.isEmpty() == false)
			open.addLast(root);

		while (open.isEmpty() == false) {
			Node parent = open.removeFirst();
			build_overwriteChildOffset(offsetMap.get(parent), o - head.length,
					sizeChildOffset, trieBytes);
			for (int i = 0; i < parent.children.size(); i++) {
				Node c = parent.children.get(i);
				boolean isLastChild = (i == parent.children.size() - 1);
				offsetMap.put(c, o);
				o = build_writeNode(c, o, isLastChild, sizeNoValuesBeneath,
						sizeChildOffset, trieBytes);
	...
	private void build_overwriteChildOffset(int parentOffset, int childOffset,
		...
		int flags = (int) trieBytes[parentOffset]
				& (BIT_IS_LAST_CHILD | BIT_IS_END_OF_VALUE);
		BytesUtil.writeUnsigned(childOffset, trieBytes, parentOffset,
				sizeChildOffset);
		trieBytes[parentOffset] |= flags;
	}

	private int build_writeNode(Node n, int offset, boolean isLastChild,
			int sizeNoValuesBeneath, int sizeChildOffset, byte[] trieBytes) {
		int o = offset;
		...
		// nValuesBeneath
		BytesUtil.writeUnsigned(n.nValuesBeneath, trieBytes, o,
				sizeNoValuesBeneath);
