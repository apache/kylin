...
select SELLER_ID,kcg.CATEG_LVL3_ID,kcg2.CATEG_LVL3_ID ,sum(price),sum(case when TRANS_ID > 10 then price else 1 end)
,count(distinct TEST_KYLIN_FACT.ORDER_ID),count(distinct case when TRANS_ID > 10 then price else 1 end)
...
left join KYLIN_CATEGORY_GROUPINGS as kcg on TEST_KYLIN_FACT.LEAF_CATEG_ID = kcg.LEAF_CATEG_ID
