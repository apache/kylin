...
public class RawTableRecord implements Cloneable {
	...
	protected void setValueID(int col, int id) {
		BytesUtil
				.writeUnsigned(id, buf, digest.offset(col), digest.length(col));
	}

	protected int getValueID(int col) {
		return BytesUtil.readUnsigned(buf, digest.offset(col),
				digest.length(col));
	}
	...
	public void setValueBytes(int col, ImmutableBytesWritable bytes) {
		System.arraycopy(bytes.get(), bytes.getOffset(), buf,
				digest.offset(col), digest.length(col));
	}
