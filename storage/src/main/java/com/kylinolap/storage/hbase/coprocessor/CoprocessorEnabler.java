...
public class CoprocessorEnabler {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(CoprocessorEnabler.class);
	...
	public static ResultScanner scanWithCoprocessorIfBeneficial(
			CubeSegment segment, Cuboid cuboid, TupleFilter tupleFiler,
			Collection<TblColRef> groupBy,
			Collection<RowValueDecoder> rowValueDecoders,
			StorageContext context, HTableInterface table, Scan scan)
			throws IOException {

		if (!isCoprocessorBeneficial(segment.getCubeInstance(), groupBy,
				rowValueDecoders, cuboid, context)) {
			return table.getScanner(scan);
		}

		SRowType type = SRowType.fromCuboid(segment, cuboid);
		SRowFilter filter = SRowFilter.fromFilter(segment, tupleFiler);
		SRowProjector projector = SRowProjector.fromColumns(segment, cuboid,
				groupBy);
		SRowAggregators aggrs = SRowAggregators
				.fromValuDecoders(rowValueDecoders);

		if (DEBUG_LOCAL_COPROCESSOR) {
			RegionScanner innerScanner = new RegionScannerAdapter(
	...
	private static boolean isCoprocessorBeneficial(CubeInstance cube,
			Collection<TblColRef> groupBy,
			Collection<RowValueDecoder> rowValueDecoders, Cuboid cuboid,
		...
		HashSet<TblColRef> aggrCols = new HashSet<TblColRef>(
				cuboid.getColumns());
