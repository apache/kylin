...
public class SqlFunctions {
	...
	public static String overlay(String s, String r, int start) {
		...
		return s.substring(0, start - 1) + r
	...
	public static ByteString overlay(ByteString s, ByteString r, int start) {
		...
		return s.substring(0, start - 1).concat(r)
	...
	public static Long plus(Long b0, Integer b1) {
		return (b0 == null || b1 == null) ? null : (b0.longValue() + b1
				.longValue());
	}
	...
	public static String toString(float x) {
		...
		BigDecimal bigDecimal = new BigDecimal(x, MathContext.DECIMAL32)
				.stripTrailingZeros();
	...
	public static String toString(double x) {
		...
		BigDecimal bigDecimal = new BigDecimal(x, MathContext.DECIMAL64)
				.stripTrailingZeros();
		final String s = bigDecimal.toString();
		return s.replaceAll("0*E", "E").replace("E+", "E");
	}
	...
	public static byte toByte(Object o) {
		return o instanceof Byte ? (Byte) o
				: o instanceof Number ? toByte((Number) o) : Byte.parseByte(o
						.toString());
	}
	...
	public static BigDecimal toBigDecimal(Number number) {
		...
		return number instanceof BigDecimal ? (BigDecimal) number
				: number instanceof BigInteger ? new BigDecimal(
						(BigInteger) number)
						: number instanceof Long ? new BigDecimal(
								number.longValue()) : new BigDecimal(
								number.doubleValue());
	}

	public static BigDecimal toBigDecimal(Object o) {
		return o instanceof Number ? toBigDecimal((Number) o) : toBigDecimal(o
				.toString());
	}
	...
	public static String truncate(String s, int maxLength) {
		return s == null ? null : s.length() > maxLength ? s.substring(0,
				maxLength) : s;
	}
	...
	public static int ymdToJulian(int year, int month, int day) {
		...
		int y = year + 4800 - a;
		int m = month + 12 * a - 3;
		int j = day + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400
				- 32045;
		if (j < 2299161) {
	...
	private static void fraction(StringBuilder buf, int scale, long ms) {
		if (scale > 0) {
			buf.append('.');
			long v1 = scale == 3 ? ms : scale == 2 ? ms / 10
					: scale == 1 ? ms / 100 : 0;
